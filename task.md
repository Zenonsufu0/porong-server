...
| 데이터 수집 공백 (운영 판단용) — 7종 중 1종 해소 | 검토일 2026-05-30 (→ idea_inbox INBOX-004). **판단 가능**: 보스 통계·PvP·경매(DB 영속). **✅ #1 리텐션 구현 완료 (DL-078)** — `player_session_log` + `/api/v1/activity/summary`·`/dau`. **남은 공백**: ②골드 흐름(`addEconomyFlow` 호출 0건, 미수집) ③강화 로그(in-memory 휘발) ④보스 파티 스펙(placeholder 0/NULL, DL-064) ⑤보스 데미지 기여(`damage_share` 테이블 없음) ⑥PvP 무기/IL(미저장) ⑦성장 시계열(JSON 덮어쓰기). 죽은 모델: `EconomyFlowRecord`·`MarketPricePoint`·`LifeResourceSupplyRecord`(write 0건) |
