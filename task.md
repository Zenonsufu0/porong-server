...
| 우선도 | 항목 | 비고 |
|---|---|---|
| 중간 | 코드↔기획 불일치 후속 (INBOX-005) | 🔴 2건 모두 해소: 강화 테이블(DL-086)·보스 시드(DL-087). **잔여(server-config)**: MM `fallen_knight` 필드/시즌 셸 충돌, 배포 사본(강화표·boss_master) 동기화. 🟠 6건(영지 오프라인생산·흔적미연동·스폰스케줄러 등)은 INBOX-005 |
