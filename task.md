...
| 데이터 수집 공백 7종 (운영 판단용) | 검토일 2026-05-30 (→ idea_inbox INBOX-004). **판단 가능**: 보스 통계·PvP·경매(DB 영속). **판단 불가**: ①리텐션/플레이타임(접속 로그 테이블 없음) ②골드 흐름(`addEconomyFlow` 호출 0건, 미수집) ③강화 로그(in-memory 휘발) ④보스 파티 스펙(placeholder 0/NULL, DL-064) ⑤보스 데미지 기여(`damage_share` 테이블 없음) ⑥PvP 무기/IL(미저장) ⑦성장 시계열(JSON 덮어쓰기). 죽은 모델: `EconomyFlowRecord`·`MarketPricePoint`·`LifeResourceSupplyRecord`(write 0건). 구현 우선순위 미확정 |
