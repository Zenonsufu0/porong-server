...
public final class CurrencyUtil {
    ...
    public static int balance(Player player) {
        ...
        for (int i = 0; i < inv.items.size(); i++) {
            ...
            if (s.is(ZenonGunItems.COIN.get())) {
                total += s.getCount();
            }
        }
        return total;
    ...
    public static void give(Player player, int amount) {
        int max = ZenonGunItems.COIN.get().getMaxStackSize(new ItemStack(ZenonGunItems.COIN.get()));
        while (amount > 0) {
            int n = Math.min(amount, max);
            ItemStack coins = new ItemStack(ZenonGunItems.COIN.get(), n);
