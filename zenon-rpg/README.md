...
| **디스코드 온보딩** | 인증 게이트 → 역할 부여, 직업 역할, 필드보스 알림 (봇은 `../zenon-discord/`). |
...
> 사전 준비: JDK 21, Paper 1.21.10 서버. (Discord 봇은 `../zenon-discord/` 참조.)
...
- 템플릿: `env/application.example.env`, `env/database.example.env` (봇은 `../zenon-discord/.env.example`).
