...
> **[STATUS: DRAFT]** — Phase 1은 `/rpg-admin` 명령으로 구현 완료. Phase 2 항목은 1차 시즌 운영 피드백 후 우선순위 재조정.
...
| 작위 강제 승급/강등 | ✅ 좌클릭=▲ / 우클릭=▼ (ordinal ±1 clamp). 명령 `/rpg-rank`도 존재 |
| 멤버 강제 제거 | ⏳ 보류 — 초기화로 전체 멤버 제거는 가능, 개별 제거 GUI는 후속 |
| 자원 강제 지급/회수 | ⏳ 보류 — `/rpg-currency`·`/rpg-give` 명령 존재 (영지 customItem 직접 조작 GUI는 후속) |

- GUI: `AdminTerritoryGui` (54슬롯, 목록+클릭 액션). 진입: `/rpg-admin` → slot 29.
- 명령어: 신규 없음 — 액션은 기존 `/rpg-rank`(작위)·`/rpg-island-reset`(소셜 초기화)와 동일 (C 방식: GUI 액션 ↔ 명령 동등). 초기화 로직은 `resetSocialSettings`로 공통화(DRY).
...
진입 경로:
- GUI: `/rpg-admin` → slot 31 (운영 토글)
- 명령어: `/rpg-toggle <flag> [on|off]`, `/rpg-toggle list`
...
- GUI: `AdminLogGui` (54슬롯, 3탭 강화/거래/PvP, 페이지네이션 45/page, 읽기 전용). 진입: `/rpg-admin` → slot 33.
- 명령어: `/rpg-log [enhance|trade|pvp]` (콘솔 가능, 탭별 최근 10건 텍스트).
...
| 강제 종료 | ✅ 좌클릭/`/rpg-boss-end` → `endRun(runId, false, "admin_force")` → onRunEnded → `releaseByRunId` (슬롯+참가자 해제) |
| 강제 페이즈 트리거 | ⏳ 보류 — `BossPatternScheduler.enqueueForced` 연동은 Step 4+ |

- GUI: `AdminBossGui` (54슬롯, 진행 중 런 목록, 좌클릭=강제 종료). 진입: `/rpg-admin` → slot 24.
- 명령어: `/rpg-boss-list` (목록), `/rpg-boss-end <runId 앞 8자리>` (강제 종료, 접두어 매칭). 한 핸들러 `AdminBossCommand`가 라벨 분기.
...
| 기능 | 비고 |
|---|---|
| 직업 변경 | `/rpg-setclass` 이미 있음 — GUI 통합 검토 |
...
- `/rpg-admin` 권한: `zenon.rpg.admin` (모든 하위 권한 포함)
