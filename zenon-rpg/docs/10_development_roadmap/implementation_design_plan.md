...
| 구분 | 작성 대상 | 기준 문서 |
|---|---|---|
| 아이템·재화 키 | item_id, currency key, PDC key, CMD | `zenon-rpg/docs/02_database_api_stats/item_master_v1.md` |
| 클래스 선택 | GUI slot, 지급 장비, 첫 접속 상태 전이 | `zenon-rpg/docs/02_database_api_stats/item_master_v1.md` |
| 스킬 입력 | LMB/RMB/Shift+RMB/F 매핑, 이벤트 처리 | `zenon-rpg/docs/04_combat_weapon_skills/weapon_skills_v1.md` |
| 필드 드랍 | MythicMob 태그 규칙, 보상 지급 경계 | `zenon-rpg/docs/06_fields_bosses/drop_tables_v1.md` |
| 저장·복원 | JSON schema v3, migration 정책 | `PlayerSaveData.java` |
| GUI 라우팅 | title 상수, slot 매핑, listener 연결 | `zenon-rpg/docs/08_resourcepack_pipeline/gui_hub_structure.md` |
...
영지 저장고(`island_storage`)는 `zenon-rpg/docs/05_island_farm_system/island_system_design.md` 기준 DB 가상 저장고로 분리한다. Player JSON의 `storage` 필드는 제거하거나 migration 전용 legacy 필드로만 취급한다.
