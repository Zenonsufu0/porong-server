...
> **[STATUS: REFERENCE]** — 현재 CANON 기준의 구현 진입점. 구버전 장문 구현 메모는 `zenon-rpg/docs/_archive/implementation_reference_legacy.md`로 이동됨.
...
| 2 | `CANON.md` | 플러그인 경계, ZenonRPG 책임, 부트스트랩 순서 |
...
ZenonRPG는 전투, 장비, 영지, 보스, 보상, DB, API, Discord/web 연동 데이터를 소유한다.
...
- 구버전 상세를 참고해야 하면 `zenon-rpg/docs/_archive/implementation_reference_legacy.md`를 읽되, 현재 기준과 비교한 뒤 필요한 내용만 별도 문서로 승격한다.
