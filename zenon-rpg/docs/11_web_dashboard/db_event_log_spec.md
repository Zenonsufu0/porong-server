...
> **[STATUS: DRAFT]** — `zenon_rpg.db`에 추가할 이벤트 로그 테이블 설계.
>
> 기준일: 2026-05-23
>
> 전제: 기존 `zenon_rpg.db` 구조와 충돌하지 않도록 신규 테이블만 추가한다.
...
| 테이블 | 역할 | INSERT 주체 |
|---|---|---|
| `gold_event_log` | 골드 생성·소모 원시 로그 | ZenonRPG 이벤트 발생 시 |
| `enhancement_stone_log` | 강화석 적립·소모 원시 로그 | ZenonRPG 이벤트 발생 시 |
| `cube_event_log` | 큐브 조각·큐브 흐름 원시 로그 | ZenonRPG 이벤트 발생 시 |
| `item_issue_log` | 잠재 확정·전승권 원시 로그 | ZenonRPG 이벤트 발생 시 |
| `boss_clear_log` | 보스 전투 결과 | ZenonRPG 보스 종료 시 |
| `boss_combat_log` | 보스 전투 개인 DPS | ZenonRPG 보스 종료 시 |
| `server_perf_log` | TPS·핑·동접 수 시간별 스냅샷 | ZenonRPG 1분 주기 스케줄러 |
| `field_activity_log` | 필드별 처치 수·자원 생산 원시 로그 | ZenonRPG 몹 처치 이벤트 발생 시 |
| `daily_economy_snapshot` | 일별 집계 스냅샷 | ZenonRPG 메모리 누적 + 10분 플러시 (DL-060) |
| `bug_report` | 디스코드 버그 제보 원시 로그 | ZenonRPG API 접수 시 |
...
디스코드 봇 `/버그제보` 명령어를 ZenonRPG API가 접수할 때 INSERT된다. `id` AUTOINCREMENT가 공개 접수번호(`BUG-{id}`)로 사용된다.
...
- 라이브 `zenon_rpg.db`에는 **현재 시즌 + 시즌 종료 후 14일** 원시 로그만 유지한다.
