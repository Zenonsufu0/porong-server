...
### DL-132 (2026-06-05) — 포로서버 중앙제어 디스코드 봇 구조 확정 + 스택 정정(DL-030 SUPERSEDE)

**배경:** `poro-discord/` 봇이 RPG 전용에서 **포로서버 전체 운영 허브**로 자리잡았다. 코드/문서가 성숙(`docs/` 7종 + `core`/`integrations`/`modules` 도메인 분리)했으나 SoT 결정 기록이 DL-030(설계 3종)·DL-029(버그번호)에 머물러 있어, 현행 구조를 정본 결정으로 고정하고 DL-030의 스택 결정을 정정한다.

**결정:**
1. **봇 역할 = 중앙제어 허브.** RPG·포로몬 등 멀티 게임 서버의 조회/역할/알림을 통합. 실제 게임 로직은 각 서버가 처리하고, 봇은 **명령어·권한·알림·조회·운영자 UI**만 담당.
2. **도메인 격리 3계층 구조 고정** — `core/`(도메인 비종속 공통: config·permissions) · `integrations/<도메인>_api.py`(외부 서버 통신, discord 객체 미사용) · `modules/<도메인>/`(Cog). RPG↔포로몬 상호 import 금지, 공통은 `core/`로 승격. 신규 도메인은 `modules/<도메인>/` + (필요 시) `integrations/<도메인>_api.py` 추가 후 `main.py`의 `EXTENSIONS`에 등록.
3. **권한 역할(수동) ↔ 알림 역할(자동) 분리** — 알림 역할(`NOTIFY_ROLE_IDS`)은 `/알림설정` 등으로 자유 토글하되 어떤 운영 권한도 부여하지 않음. 권한 역할(`PERMISSION_ROLE_IDS`: Owner/Admin/RPG·Poromon·Event Manager/Support)은 운영진이 디스코드에서 직접 지급하며 **봇은 버튼·이모지·자동 로직으로 절대 지급하지 않는다.** 온보딩 승급(미인증→접속대기→인증유저)만 봇 자동.
4. **구현 스택 = Python 3.12 / discord.py 2.3** — **DL-030 ③ "Node.js(Discord.js)" 결정을 SUPERSEDE.** 실제 구현이 Python으로 진행됐고 게임 서버 독립 프로세스라는 DL-030의 본래 의도(재시작이 게임에 무영향)는 그대로 충족. `docs/domains/rpg.md §8`의 Node.js 표기는 후속 정합 대상.
5. **연동/운영 명령어 = 명시 요청 시에만 실구현.** 외부 서버 직결 API, 계정 연동·역할 지급·관리자 명령어는 보안 영향이 커 인터페이스/설계 선행이 원칙(`integrations/poromon_api.py`·`modules/admin` 스텁 유지).

**파일:**
- `poro-discord/docs/{index,architecture,roles_and_permissions,notifications}.md`, `docs/domains/{rpg,poromon,common,admin}.md` (현행 구조 기술)
- `poro-discord/CLAUDE.md` (작업 영역 지침)
- `poro-discord/docs/task.md` (신규 — 구현 작업 목록, 본 DL과 연동)

**이유:** 봇이 멀티 서버 허브가 되면서 도메인 격리·권한/알림 분리·연동 게이팅이 보안·유지보수의 핵심이 됨. 산재한 현행 구조를 정본으로 고정해 후속 작업(notifier·admin·poromon)이 동일 원칙 위에서 진행되도록 한다. 스택은 "결정된 적 있으나 실제와 어긋난" 대표 사례라 SUPERSEDE를 명시.

**남은 작업(→ `poro-discord/docs/task.md`):** 통합 알림 디스패처 `core/notifier.py` 설계, `modules/admin`·`event`·`poromon` 실구현(설계 선행), `integrations/poromon_api.py` 실구현, `rpg.md §8` Node.js 표기 정합.

**근거:** 사용자 확인 (2026-06-05).

---

### DL-133 (2026-06-05) — 봇 오라클 상시호스팅 + 멀티서버 온보딩(공통 디스코드 인증 → 서버별 약관동의 → 서버별 화이트리스트)

**배경:** 디스코드 봇은 24시간 상시 운영이 전제(알림·온보딩 허브). 또한 RPG·포로몬 양쪽을 다루는 중앙제어 봇이 되면서 기존 RPG 단일 온보딩(`미인증→접속대기→인증유저`, 인게임 `/연동` 코드)을 멀티서버 모델로 재정의할 필요가 생겼다.

**결정:**
1. **호스팅 분리.** 봇은 **게임 서버 호스팅과 분리된 오라클 클라우드 인스턴스**에 배포해 24h 상시 운영한다. 게임 서버(RPG/포로몬) 재시작·점검이 봇 가동에 무영향(독립 프로세스·독립 호스트). 봇→게임 서버는 `integrations/*_api.py` HTTP 경유로만.
2. **2단계 온보딩 모델.**
   - ① **공통 디스코드 인증(도메인 비종속, 1회):** 디스코드 입장(`미인증`) → 서버 규칙/이용약관 동의 + **마인크래프트 닉네임 1회 입력** → `접속대기`(디스코드 인증 완료). 공통 인증만으로는 어떤 게임 서버에도 접속 불가.
   - ② **서버별 약관동의 + 화이트리스트(도메인별):** 인증 유저에게 RPG·포로몬 **각 서버 약관동의 패널** 노출 → 동의한 서버에 한해 인게임 `/연동 <코드>` 본인인증 → **해당 서버 화이트리스트 등록 + 서버별 플레이어 역할** 부여. 동의·인증한 서버에만 접속.
3. **닉네임 = 공통 1회 입력.** RPG·포로몬 동일 마크 계정 가정. ①에서 받은 닉네임을 ②의 서버별 화이트리스트 등록에 재사용(사용자 확정).
4. **본인인증(`/연동` 코드) = 서버별 유지.** 약관동의만으로 자동 등록하지 않고, 인게임 `/연동` 코드로 닉네임 소유권을 검증한 뒤 화이트리스트 확정(사칭 방지). 현행 RPG 방식을 서버별로 계승(사용자 확정).
5. **역할 매핑.** 현행 `인증유저` 역할 = **RPG 화이트리스트(RPG 플레이어) 역할**로 의미 명확화(신설 없음). 포로몬은 동일 패턴의 `포로몬플레이어` 역할 신규 설계. 공통 인증 역할(`미인증`·`접속대기`)은 도메인 비종속.

**파일:**
- `poro-discord/docs/roles_and_permissions.md` — 대원칙 ④ 추가, §A(공통 온보딩)·§D(서버별 화이트리스트) 신규/재정의.
- `poro-discord/docs/architecture.md` — "배포/호스팅"·"공통 온보딩 vs 서버별 화이트리스트" 절 + 공통 인증 분리 TODO.
- `poro-discord/docs/domains/rpg.md §2` — 온보딩 플로우를 멀티서버 ②의 RPG 분기로 정합.
- `poro-discord/docs/index.md` — 구현 현황표(멀티서버 온보딩·배포) 추가.
- `poro-discord/docs/task.md` — T7(공통 인증 `core/` 분리)·T8(오라클 배포)·T9(포로몬 온보딩) 추가.

**이유:** 봇이 멀티 게임 서버 허브가 되면서 "디스코드 신원 인증"과 "게임 서버 접근 권한(화이트리스트)"을 분리해야 한다. 공통 인증으로 디스코드 신원을 한 번 확인하고, 실제 게임 접근은 각 서버 약관동의 + 인게임 본인인증을 거친 서버에만 부여 → 약관 책임을 서버별로 분리하고, 한 서버만 하는 유저가 다른 서버 약관에 묶이지 않게 한다. 상시성은 알림·온보딩이 게임 서버 점검과 무관히 동작해야 하므로 호스팅 분리가 필요.

**구현 상태:** ②의 RPG 분기·인게임 `/연동`은 구현됨. ①의 공통 인증 코드 분리(`core/`), 포로몬 ② 분기, 오라클 배포는 미구현(→ `task.md` T7·T8·T9).

**관련:** DL-132(봇 구조 정본·스택), DL-030(구 RPG 온보딩 일부 계승), DL-029(`/연동` 인증코드 체계). 코드 변경 없음 — 설계 결정·docs 정합만.

**근거:** 사용자 확인 (2026-06-05).

---

### DL-134 (2026-06-05) — 카테고리 채널 구조 + 이모지 서버선택(역할기반 가시성) + 운영자 닉네임 변경 (DL-133 후속)

**배경:** DL-133 멀티서버 온보딩의 채널/역할 구현 방식을 구체화. 또한 DL-133에서 미정으로 남긴 "다른 마크 계정으로 플레이하려는 경우"의 닉네임 변경 통제 방식을 확정.

**결정:**
1. **카테고리 단위 채널 구조.** 디스코드 서버를 **공통 / RPG / 포로몬 / 기타** 카테고리로 묶는다. 각 게임 카테고리 안에 **약관 채널 + 정보/소통 채널**을 둔다.
2. **이모지 서버선택 → 카테고리 접근 역할 → 역할기반 가시성.** 공통 인증을 마친 유저가 **서버선택 패널에서 플레이할 서버를 이모지로 선택** → 봇이 `RPG접근`·`포로몬접근`·`기타접근` 등 **카테고리 접근 역할**을 자동 부여. 디스코드 채널 권한이 이 역할에게만 카테고리를 노출 → 선택한 서버 카테고리(약관 채널 포함)가 보인다. 이모지 해제 시 역할 회수.
3. **카테고리 접근 ≠ 화이트리스트.** 카테고리 접근 역할은 카테고리를 **보이게만** 한다(운영 권한 없음 → 자동 부여 허용, 대원칙 ③). 실제 게임 접속(화이트리스트)은 그 안 약관 채널에서 약관동의 + 인게임 `/연동` 인증을 마쳐야 부여(DL-133 §D). → 역할 정책 대원칙 ⑤ 신설.
4. **확장 규칙.** 새 게임/서버 추가 = `카테고리 1개 + <도메인>접근 역할 + 선택 이모지 + (도메인 모듈)` 한 세트.
5. **닉네임 변경 = 운영자 전용.** 닉네임은 공통 인증 시 1회 입력(DL-133), 플레이어 셀프 변경 금지. 다른 마크 계정 전환은 **운영자 문의 → 운영자 명령**으로만: `/닉네임변경 <유저> <새닉네임> <사유>`(직접 교체) 또는 `/닉네임재입력 <유저>`(본인 재입력 허용 토글). 교체는 화이트리스트 정합(구 닉네임 해제/신 닉네임 재인증) + 운영로그 기록 동반, 게임 서버 API 경유.

**파일:**
- `poro-discord/docs/roles_and_permissions.md` — 대원칙 ③ 갱신·⑤ 신설, §A-2(서버 선택=카테고리 접근 역할) 신규.
- `poro-discord/docs/architecture.md` — "디스코드 채널/카테고리 구조" 절 신규.
- `poro-discord/docs/domains/rpg.md §2` — 온보딩 플로우에 이모지→카테고리 가시화 단계 추가.
- `poro-discord/docs/domains/admin.md` — `/닉네임변경`·`/닉네임재입력` 명령 + 닉네임 변경 정책.
- `poro-discord/docs/task.md` — T10(이모지/카테고리 가시성)·T11(운영자 닉네임 변경) 추가.

**이유:** 카테고리+역할 가시성은 디스코드 네이티브 기능으로 "관심 서버만 보이는" UX를 구현하면서, 화이트리스트 게이트(약관+인게임 인증)는 그대로 분리 유지한다. 닉네임은 화이트리스트·정산의 키이므로 셀프 변경을 막고 운영자 통제 + 감사 로그로 한정해 사칭·중복·정합 깨짐을 방지.

**구현 상태:** 미구현(설계 결정·docs 정합만). T10(이모지/카테고리), T11(닉네임 변경 명령) → `task.md`. 코드 변경 없음.

**관련:** DL-133(멀티서버 온보딩·호스팅), DL-132(봇 구조), DL-029(`/연동` 인증코드).

**근거:** 사용자 확인 (2026-06-05).

---

### DL-135 (2026-06-05) — 서버 구조 + 봇 관여 경계 + 통신 패턴(게임서버→봇 push) 확정

**배경:** 봇이 "어디까지 관여하는가"의 경계와, 멀티 게임서버(RPG Paper / 포로몬 Cobblemon+PoroMonCore)와의 통신 방식을 확정. DL-132~132(봇 구조·온보딩·채널)에 이은 인프라/연동 설계.
...
**이유:** "디스코드 신원/UX"와 "게임 상태"의 권위를 분리하면 게임서버 점검/재시작이 봇과 독립(오라클 분리 의도, DL-133)이고, 보안 사고 표면이 정의된 API로 한정된다. 포로몬을 RPG와 같은 HTTP API로 맞추면 봇 도메인 모듈·integrations 패턴이 일관된다. push는 폴링 지연·낭비를 없애고 도메인 확장 시 일관된 알림 경로를 준다.
...
**관련:** DL-132(봇 구조·스택), DL-133(호스팅 분리·온보딩), DL-134(채널/역할), DL-030(/강화계산 내부계산).

**근거:** 사용자 확인 (2026-06-05).

---

### DL-136 (2026-06-05) — 봇↔게임서버 통신 계약 명세 (DL-135 구체화)

**배경:** DL-135에서 통신 방향(봇→게임서버 API, 게임서버→봇 push)을 확정. 이를 엔드포인트·페이로드 수준 계약으로 구체화. 현행 RPG API(rpg_api.py 8개 엔드포인트)를 정본화하고, push 이벤트·운영 API·포로몬 연동을 설계로 명세.
...
2. **방향 A(봇→게임서버) 공통 규약:** JSON over HTTP(S), `X-Api-Key` 헤더(서버별 키, .env), 도메인별 베이스 URL(`PORO_API_URL`/신규 `POROMON_API_URL`), 에러 `{error,message}`, 운영 명령 멱등성 고려.
   - A-1 인증/온보딩(RPG 🟢): `/auth/pending`·`/auth/status/{nick}`·`/auth/role-queue`·`/auth/role-granted`. 화이트리스트는 봇이 요청만, 게임서버가 확정(DL-133 §D).
...
**관련:** DL-135(통신 패턴), DL-133(온보딩·화이트리스트), DL-134(닉네임/화이트리스트 운영), DL-132(/강화계산 내부계산 예외), DL-029(인증코드).
...
**번호 메모:** 디스코드 브랜치가 이 공유 원장에 **DL-132~136**(봇 구조·멀티서버 온보딩·카테고리/닉네임·통신 패턴·통신 계약)을, master가 **DL-137**(porong-economy/gun 구상)을 이미 선점했다(브랜치 미머지 상태라 rpg 워크트리에서는 안 보였음). 전역 충돌 회피로 본 항목은 **DL-138**을 부여한다. 본 DL은 그 06-05 디스코드 결정들(특히 DL-133 공통 인증·DL-136 통신 계약)을 06-08 "코드 방향 통일 + RPG 측 구현"으로 잇는다.
...
### DL-140 (2026-06-13) — Zenon 2차 전환: in-repo 폴더명 `porong-*` → `zenon-*` rename

**배경:** DL-139(1차)에서 표시명·문서·worktree 명칭만 Zenon으로 정리하고 in-repo 폴더명 `porong-*` rename은 보류했다. 개발 재개 전 이름 부채를 줄이기 위해 2차로 폴더명을 rename한다. 사용자 지시(2026-06-13).

**무엇 (확정):** monorepo in-repo 최상위 프로젝트 폴더 5개를 `git mv`로 rename(master):
- `porong-rpg/` → `zenon-rpg/`
- `porong-mon/` → `zenon-mon/`
- `porong-discord/` → `zenon-discord/`
- `porong-economy/` → `zenon-economy/`
- `porong-gun/` → `zenon-gun/`

**표시명:** Zenon RPG / Zenon Mon / YUKI-01 · Zenon Discord / Zenon Economy / Zenon Gun.

**경로 참조 갱신 (라이브 문서·설정):** `README.md`·`AGENTS.md`·루트 `CLAUDE.md`·각 프로젝트 `CLAUDE.md`·`.gitignore`(`!zenon-rpg/docs/_archive/` 등 ignore 규칙)·`docs/worktree_policy.md`·SoT `final_master_plan.md`·살아있는 설계 문서의 내부 self-link(`zenon-rpg/docs/...` 등)를 새 폴더명으로 치환. 서버 표시명 `포로 서버`/`Poro Server` → `Zenon 서버`/`Zenon Server`(문서 .md 한정), 디스코드 `포로서버`/제목 `Poro Discord Bot` → Zenon 정리.

**변경하지 않은 것 (코드 식별자 — 별도 후속 제안):**
- Java 패키지 `com.poro.rpg`·`kr.poro.poromoncore`, 플러그인 빌드 디렉토리 `custom-plugins/poro-rpg`, 커스텀 모드 디렉토리 `custom-mods/poromon-core`, 플러그인명 `PoroRPG`·모드명 `PoroMonCore`, 모드/제품명 `PoroMon`/`포로몬`, 코드 모듈 `modules/poromon`·`integrations/poromon_api.py`, 명령어 `/poromon`, 문서 하위폴더 `03_poromoncore`, mod id/config key/assets 네임스페이스.
- 이유: 5개 폴더 토큰(`porong-*`)과 위 식별자(`poro*`)는 문자열이 겹치지 않아 폴더 rename이 식별자를 건드리지 않는다. 코드 식별자 rename은 빌드·런타임·DB·리소스팩 영향이 크므로 **폴더 rename과 다른 커밋·별도 결정**으로 분리(사용자 지시).
- 런타임/config 표시명(`config.yml` `server-name: "Poro Server"`, `plugin.yml` description, 스코어보드 `§6포로 서버`, `build.gradle` description, 봇 `main.py`/`auth.py` 문자열)도 코드/런타임 영역이라 미변경 — 제안 대상.

**이력 보존(DL 정책):** 본 결정 로그(`decision_log.md`)와 `idea_inbox.md`·`_archive/`의 **과거 경로·브랜드 표기는 당시 값 그대로 보존**(일괄치환 제외). DL-130/DL-131/DL-137/DL-139 등 이전 항목의 `poro-*`/`porong-*` 표기는 의도적으로 유지된 과거 기록이다. `worktree_policy.md §7`의 과거 전환 메모도 당시 값으로 복원했다(sed 부작용 정정).

**전환기 후속 (필수):**
- 세 feature 브랜치(`feature/rpg-dev`/`feature/poromon-dev`/`feature/discord-dev`)는 아직 옛 폴더명(`porong-*`). 각 브랜치가 본 rename을 머지하기 전까지 그 worktree의 `git sparse-checkout set` 대상은 여전히 `porong-*`여야 빈 worktree가 안 된다. **머지 후** 각 worktree에서 `git sparse-checkout set zenon-<프로젝트>`로 재설정한다(`worktree_policy.md §3`).
- 선행 stale 절대경로(`~/dev/poro-server*`·`poro-server-poromon`: `zenon-mon/scripts/*`·`server_runbook.md`·`zenon-rpg/docs/08_resourcepack_pipeline/*`·`task.md`)는 DL-131이 이미 "별도 stale 정리 대상"으로 명시한 선행 부채 — 본 rename 범위 밖, 별도 정리.
