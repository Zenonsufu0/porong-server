...
### §3.2 PoroRPG 부트스트랩 순서 (~25줄) ➡️ MOVE

**이동 대상:** `docs/01_plugin_architecture/CANON.md`  
**이유:** 구현 상세 (Bootstrap 8단계)는 플러그인 아키텍처 문서 소관.  
MASTER_PLAN에는 "PoroRPG가 8단계 Bootstrap 구조로 초기화된다" 한 줄만 남김.
...
IridiumSkyblock = 껍데기, PoroRPG = 핵심 로직. "스카이블럭" 표현 금지. 핵심 방향성.
...
MASTER_PLAN에는 "`/poro` 계층 커맨드, 권한 `poro.admin`" 1줄 유지.
...
| §8.1 영지 구조 원칙 | 5 | IridiumSkyblock + PoroRPG 역할 분리 |
...
| PoroRPG 부트스트랩 (§3.2) | `01_plugin_architecture/CANON.md` | ~25 |
...
| 내용 | 중복 위치 |
|---|---|
| 강화 테이블 1~25강 | `final_master_plan.md`의 "전투와 장비 성장" = `economy_numbers_v2.md` 강화 비용표 (골드 비용 불일치 포함) |
| 잠재 라인별 이탈 확률 표 | `final_master_plan.md`의 "전투와 장비 성장" = `potential_options_v1.md` |
| 잠재 옵션 풀 상세 표 | `final_master_plan.md`의 "전투와 장비 성장" = `potential_options_v1.md` |
| 공방 레시피 체인 | `final_master_plan.md`의 "개인 영지" = `workshop_crafting_spec.md` |
| 영지 작위 표 | `final_master_plan.md`의 "개인 영지" = `island_system_design.md` |
| 시설 산출량 표 | `final_master_plan.md`의 "개인 영지" = `island_system_design.md` |
| GUI 슬롯 매핑 전체 | `final_master_plan.md`의 "GUI와 리소스팩" = `08_resourcepack_pipeline/gui_*.md` (17개) |
| 보스 패턴 10종 | `final_master_plan.md`의 "필드, 보스, 드랍" = `07_boss_pattern_modules/` |
| 개발 Phase 표 | `final_master_plan.md`의 "개발 로드맵" = `10_development_roadmap/index.md` |
| 관리자 커맨드 | `final_master_plan.md`의 "운영 명령어" = `01_plugin_architecture/admin_command_spec.md` |
...
| 아이템 이름 변경권 가격 | 구 `final_master_plan` 상점 항목 300,000G vs M-11 확정 10,000G | **M-11 확정**: 10,000G — 상점 항목 수정 필요 |
