...
public final class ZenonHttpServer {
    ...
    private ZenonHttpServer(HttpServer server, ExecutorService executor, DomainLogger logger) {
        this.server = server;
        this.executor = executor;
        this.logger = logger;
    }

    public static ZenonHttpServer create(
        ...
        return new ZenonHttpServer(server, executor, logger);
    }

    public void start() {
        server.start();
        logger.info("Zenon HTTP API started on " + server.getAddress() + " — /api/v1/boss/*");
    }

    public void stop() {
        server.stop(1);
        executor.shutdownNow();
        logger.info("Zenon HTTP API stopped.");
    }
}
