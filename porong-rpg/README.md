...
| **디스코드 온보딩** | 인증 게이트 → 역할 부여, 직업 역할, 필드보스 알림 (봇은 `../porong-discord/`). |
...
| 봇 | Python **3.12+**, discord.py (`../porong-discord/`) |
...
> 사전 준비: JDK 21, Paper 1.21.10 서버. (Discord 봇은 `../porong-discord/` 참조.)

```bash
# 플러그인 빌드
cd custom-plugins/poro-rpg
./gradlew build
# 산출물: build/libs/porong-rpg-0.1.0.jar  → server/plugins/ 에 배치
```
...
- 템플릿: `env/application.example.env`, `env/database.example.env` (봇은 `../porong-discord/.env.example`).
