...
- 서버: tmux `poro` 세션에서 기동(8080 리소스팩 http 별도). 배포 = `cp build/libs/porong-rpg-0.1.0.jar server/plugins/` → tmux `save-all`→`stop`→`start.sh`. ※ 인게임 검증은 server가 있는 원본 worktree에서.
