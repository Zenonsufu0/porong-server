...
>
> **2026-06-05 레포 구조 정리(폴더 이동만, 코드/데이터 무수정):** RPG 자산이 모두 `porong-rpg/` 하위로 이동 — `custom-plugins`·`server`·`server-config`·`assets`·`tools`·`scripts`·`env`·`docs`. 프로젝트 전역 SoT(`final_master_plan.md`)·`decision_log.md`·`idea_inbox.md`·`_archive/`도 `porong-rpg/docs/`로 통합(루트 `docs/` 폐지). 디스코드 docs는 `porong-discord/docs/`. orc 워크플로우 스크립트 제거. 빌드: `cd porong-rpg/custom-plugins/poro-rpg && ./gradlew build`.
...
- **플러그인 리브랜드: `EmpireRPG`/`empire-rpg` → `PoroRPG`/`porong-rpg` 전체 완료** (DL 없음, 커밋 메시지 참조). 패키지 `com.poro.rpg`, 명령어 `/poro`·`poro-*`, 권한 `poro.*`, 봇 `PORO_API_*`, 데이터 식별자(NBT·스코어보드 태그)까지. 재배포 시 `server/plugins/EmpireRPG→PoroRPG` 폴더 이전 필요.
...
- 서버(DL-130 갱신): RPG = tmux **`pororpg`** 세션 / 런타임 `porong-rpg/.local/server`(`./start.sh` = paper.jar) / 포트 **25565** / 리소스팩 http 8080. 배포 = `cp build/libs/poro-rpg-0.1.0.jar .local/server/plugins/` → `stop`→`start.sh`. ⚠️ tmux 이름 `poro`는 **poromon 서버(25566)**가 쓰므로 충돌 주의 — RPG는 반드시 `pororpg`.
...
기준 문서: `porong-rpg/docs/10_development_roadmap/implementation_design_plan.md`
...
기준 문서: `porong-rpg/docs/10_development_roadmap/implementation_plan_v2.md`
...
기준 문서: `porong-rpg/docs/04_combat_weapon_skills/CANON.md §13`, DL-077
...
기준 문서: `porong-rpg/docs/10_development_roadmap/admin_gui_phase2.md`, 커밋 `627e163`
...
| `porong-rpg/docs/10_development_roadmap/admin_gui_phase2.md` — Step 2 완료/Step 2b hook 명시 | docs | ✅ |
| `BUILD SUCCESSFUL` | — | ✅ |

## §6-18 관리자 Phase 2 Step 5 — 영지 관리 GUI (slot 29) — 완료 (2026-05-30)

기준: `porong-rpg/docs/10_development_roadmap/admin_gui_phase2.md` §영지 관리 (slot 29). **Phase 2 마지막 단계.**
...
기준: `porong-rpg/docs/10_development_roadmap/admin_gui_phase2.md` §보스 디버그 (slot 24)
...
기준: `porong-rpg/docs/10_development_roadmap/admin_gui_phase2.md` §로그/감시 (slot 33)
...
| 낮음 | 리소스팩 파이프라인 (Phase 8) | `porong-rpg/docs/08_resourcepack_pipeline/index.md` |
...
## 밸런스 분석 결과 (서브에이전트 3종) → `porong-rpg/docs/04_combat_weapon_skills/balance_review_dl128.md`
...
### 핵심 파일
- 정본: `porong-rpg/docs/02_database_api_stats/potential_options_v1.md`
