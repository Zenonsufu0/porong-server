...
### INBOX-015 레벨업 동기 부여 — 스킬 레벨 + 공용각인 레벨 해금 (⚠️ SoT 변경 사안)
- 날짜: 2026-06-06
- 출처: 유저 피드백 "레벨을 올리는 동기가 부족하다".
- **문제(확정 설계의 부작용)**: `level_stat_system_v1.md:17` "레벨별 고정 스탯 자동 증가 없음 — HP/ATK/DEF는 장비 강화로만 성장. **확정**". 즉 현재 레벨업은 파워에 0 기여 → 레벨 동기 부재는 설계 그대로의 결과. 유저 지적 타당.
- **아이디어 (검토 중)**: 레벨업에 성장 보상을 붙인다.
  1. **레벨별 스킬 레벨** — 스킬이 레벨에 비례해 강화(소폭, 상한).
  2. **공용각인 레벨 해금** — 파밍이 아니라 **레벨을 올리면 공용각인이 하나씩 해금**(예: lv30부터 단계 해금). "파밍형 각인 수집"이 아닌 "레벨 진행 = 각인 해금" 구조.
- **✅ 결정 (2026-06-06): 하이브리드 채택** — (A 선택형) + (레벨로 슬롯 확장)의 절충.
  - 레벨업 시 **공용각인 풀이 단계적으로 해금**되고, **장착 슬롯도 레벨로 확장(1→2→3칸)**.
  - 즉 플레이어는 해금된 각인 중 **현재 슬롯 수만큼 골라 장착**(전체 동시 적용 X). 레벨업 = ①새 각인 옵션 해금 + ②슬롯 증가 → 동기 충분, 파워는 슬롯 수로 통제.
  - (보류된 대안: (A)순수 선택형 1슬롯 / (B)해금 전체 동시적용 — 후반 파워 폭증으로 미채택.)
- **상세 설계 필요(후속 — 미정)**: 공용각인 풀 목록·효과·해금 레벨 테이블, 슬롯 확장 레벨(예: lv30=1칸 / lv?=2칸 / lv?=3칸), 각인 고유효과 구현 범위.
- **⚠️ SoT 충돌(반드시 확정 필요)**:
  - `final_master_plan.md §116`: "1차 시즌은 T1 단일 체계 — …**공용각인**…은 제외한다." → **공용각인 부활 = SoT 직접 변경**.
  - `level_stat_system_v1.md`: "레벨 자동성장 없음, 장비로만 성장(확정)" → 레벨→파워 도입 = 철학 완화.
  - 구분: **공용각인**(SoT 제외) vs **무기별 각인 DL-110**(코드 존재하나 "고유효과 미구현", 스택상한 토글만). 공용각인 부활 대신 무기별 각인 효과를 lv 해금하는 절충안도 검토 가치.
- **리스크/선행**: 레벨→파워가 되면 **보스 HP·필드 밸런스 재산정**(`balance_review_dl128.md`) 필요. 각인 **고유효과 구현**(현재 미구현)이 실작업. 공용각인 풀·효과·해금 레벨 테이블 신규 설계 필요.
- 상태: **방향 확정(2026-06-06) — 공용각인 부활 + 하이브리드(풀 해금 + 슬롯 1→2→3 확장)**. ⚠️ 단 상세 설계(각인 풀/효과/레벨 테이블) 미완 → 그 설계 확정 후 final_master_plan(SoT §116 "공용각인 제외" 반전)·CANON 갱신 + decision_log(DL-NNN) + 구현. 현재는 SoT 미수정(상세설계 대기).
