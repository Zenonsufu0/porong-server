...
- 플러그인 구현은 `porong-rpg/custom-plugins/poro-rpg` (기존 PoroRPG 경로).
- 런타임 서버 파일은 명시적 요청 시에만 `porong-rpg/server/plugins` 또는 `porong-rpg/server-config` 편집.
- `porong-rpg/server/`, `porong-rpg/custom-plugins/`는 명시적 요청 없이 수정하지 않는다.
