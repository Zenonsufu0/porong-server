# porong-rpg — RPG 서버 규칙

> 모노레포 전역 규칙은 루트 [`../CLAUDE.md`](../CLAUDE.md). 이 파일은 porong-rpg 프로젝트 안에서만 적용되는 RPG 전용 규칙이다.

- 프로젝트 전역 SoT는 `docs/final_master_plan.md`(= `porong-rpg/docs/`). RPG 도메인 세부는 `porong-rpg/docs/`의 각 `CANON.md`. 충돌 시 `final_master_plan.md`가 우선.
...
| Discord/onboarding | `../porong-discord/docs/index.md` (porong-discord) |

> 위 경로는 `porong-rpg/docs/` 기준. 디스코드 봇 docs만 `porong-discord/docs/`에 있다.
...
- **포로몬(`../porong-mon/`)·디스코드봇(`../porong-discord/`)은 수정 금지.** 읽기만 한다.
...
- 빌드 루트: `porong-rpg/custom-plugins/poro-rpg`. 빌드: `./gradlew build` (→ `BUILD SUCCESSFUL`).

## 코드/서버 작업 범위

- 플러그인 개발 소스는 `porong-rpg/custom-plugins/poro-rpg` (기존 PoroRPG 경로).
- 런타임 서버 실행 폴더는 `porong-rpg/.local/server`, 플러그인 배치 위치는 `porong-rpg/.local/server/plugins` (DL-130). `paper.jar`·`world`·`logs`·`plugins/*.jar`·`cache`·`versions` 등은 Git 추적 금지(`.local/` 전체 gitignored).
- 런타임 서버 파일은 명시적 요청 시에만 `porong-rpg/.local/server/plugins` 편집. MythicMobs YAML 소스(추적 대상)는 `porong-rpg/server-config`에서 편집 후 런타임에 배포한다.
- `porong-rpg/.local/server/`, `porong-rpg/custom-plugins/`는 명시적 요청 없이 수정하지 않는다.

## 에셋

- 에셋 작업: `porong-rpg/assets/source`. export 산출물: `porong-rpg/assets/export/resourcepack`.
- Blockbench 작업은 `porong-rpg/assets/source/CLAUDE.md`도 함께 따른다.
