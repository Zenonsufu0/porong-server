...
public final class FieldEventManager {
    ...
    private static void spawnEvent(MinecraftServer server, CoreConfig.FieldEvent cfg) {
        ...
        if (pool == null) {
            PoroMonCore.LOGGER.warn("[FieldEvent] 풀 없음: {}", cfg.poolId);
            return;
        }
    ...
    private static void findAndSpawn(MinecraftServer server, CoreConfig.FieldEvent cfg,
        ...
        if (attempt >= cfg.maxAttempts) {
            spawning = false;
            PoroMonCore.LOGGER.warn("[FieldEvent] 안전 위치 탐색 실패 — 다음 주기 재시도");
            return;
        }
        ...
        scm.getChunkFutureSyncOnMainThread(x >> 4, z >> 4, ChunkStatus.FULL, true).thenAccept(opt -> {
            ...
            try {
                ...
                PoroMonCore.LOGGER.info("[FieldEvent] {} Lv{} @ ({}, {}, {})", species, cfg.level, x, topY, z);
            } catch (Throwable t) {
                PoroMonCore.LOGGER.error("[FieldEvent] 스폰 실패", t);
            } finally {
                spawning = false;
            }
        });
