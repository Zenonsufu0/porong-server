...
public final class AuthManager {
    ...
    public static boolean isVerified(ServerPlayerEntity player) {
        ...
        return PoroMonState.get(player.getServer()).getOrCreate(player.getUuid()).discordVerified;
    }
    ...
    public static UUID verify(String code, String discordId) {
        ...
        s.execute(() -> {
            PoroMonState state = PoroMonState.get(s);
            ...
            PoroMonCore.LOGGER.info("[Auth] 인증 완료: {} ↔ discord {}", p.uuid(), discordId);
        });
        return p.uuid();
    }
