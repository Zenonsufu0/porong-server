...
이 저장소는 포로 서버 모노레포다. **이 작업 영역(`porong-mon/`)은 포로몬 모드서버/모드팩만 담당한다.**

### 수정 가능
- `porong-mon/` 내부 전체 (`modpack/`, `docs/`, `scripts/`, `reports/`, `CLAUDE.md`, `task.md`)

### 읽기만 (수정 금지)
- `porong-rpg/`, `porong-discord/` — 다른 프로젝트. **절대 수정하지 않는다.**
...
- Repo Layout: 포로 서버 모노레포. 포로몬 작업 영역은 모노레포 루트 하위 `porong-mon/`
  - 포로몬 전용 worktree: `/home/zenonsufu1/dev/porong-work-mon/porong-mon` (브랜치 `feature/poromon-dev`)
  - 모노레포 루트(master worktree): `/home/zenonsufu1/dev/porong-server`
  - ⚠️ 옛 경로 `/home/zenonsufu1/dev/poro-server-poromon`는 모노레포 이전 표기 — 사용 금지
- IDE: VS Code with Claude Code
- Custom Mod Path: `porong-mon/custom-mods/poromon-core` (포로몬 작업 영역 기준 `custom-mods/poromon-core`)
