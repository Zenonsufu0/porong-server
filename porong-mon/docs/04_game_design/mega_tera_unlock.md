...
- ⚠️ **골드 독점 강제(결정 042)**: MSD 기본은 메가스톤·키스톤을 크래프팅/구조물/광석으로 무료 획득시킴 → 데이터팩 `poromon_mega_control`로 MSD 구조물 5종 생성 차단(베이스 재료 공급원 = 구조물 내부 광석뿐이라 이걸로 100% 차단). 상점 골드가 유일 경로가 됨.
