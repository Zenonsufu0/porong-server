...
> 대상: **PoroMon 0.1** 클라 배포(이후 포로 모드 서버 공용). MC 1.21.1 / Fabric Loader 0.19.3 / Java 21.
> 결정 근거: **결정 046**(2026-06-12, 045 대체) — CurseForge 배제, 자체 제작 exe 설치기 + 전부 번들 + 토글 0.1부터.
> 상태: **설계 수립(2026-06-12)**. 구현은 별도(설치기는 `porong-mon` 외부 산출물 가능 — §10). 미확정은 §9.
...
유저가 **`PoroMon설치기.exe` 더블클릭 → 필수 모드 자동 + 선택 모드 체크박스 → [설치] → 게임 실행·서버 접속**. 모드는 전부 exe에 번들(인터넷·로그인 불필요). 엔진은 공용, 서버별 설정(JSON)만 교체해 재활용.
...
5. **범용 엔진 + 서버별 설정 분리** — 엔진 코드는 공용, `pack.json`만 갈아끼움(포로몬/포로건…).
...
**재활용**: 포로건은 같은 엔진에 `pack.json`(포로건 모드목록·서버주소) + 포로건 `mods/` 번들만 바꿔 빌드. 엔진은 한 곳에서 유지보수.
...
```json
{
  ...
  "server":    { "name": "PoroMon", "address": "HOST:25566" },
  ...
  "required": [
    ...
    { "file": "poromon-core-0.1.0.jar",               "name": "PoroMonCore" }
    /* … T0 코어 14 + 서버 정합 모드 … */
  ],
...
4. 진행바 → 완료 → **[게임 실행]**(또는 "공식 런처에서 PoroMon 프로필 선택" 안내).
...
7. **런처 프로필** — 공식 런처 `launcher_profiles.json`에 PoroMon 프로필(이 gameDir·Fabric 버전) 추가.
...
- **PoroMonCore·collection 등 T0/T1 정합 항목은 §결정 044 분류 준수**(PoroMonCore=required, collection=optional 강권장 default:true).
...
2. **PoroMonCore 빌드** — `custom-mods/poromon-core/gradlew build` → jar를 번들 `mods/`에 포함(서버와 동일 해시).
...
- **0.1**: 단일 엔진 + 포로몬 pack.json + 전부 번들 + 체크박스 토글 + 독립 인스턴스 + 서버 자동등록. 라이브러리 항상 포함.
- **0.2**: 선택 모드별 `requires`(의존 정교화), 업데이트 감지(설치된 버전 vs 번들), 셰이더/리소스팩 그룹 세분, 포로건 pack.json 재활용 실증.
...
- [ ] PoroMonCore 포함 + 배지/조우권/정수 텍스처 렌더(종이 아님)
- [ ] 서버 자동등록(서버 목록에 PoroMon) → 접속 성공(모드 불일치 거부 없음, T0 해시 일치)
...
1. **재배포 라이선스** — 번들=재배포. 모드별 허용 여부 확인(특히 ARR). 불가 모드 발견 시 해당만 제외/대체. **오픈 전 일괄 점검**.
...
6. **설치기 코드 위치** — `porong-mon` 내부 `installer/` vs 별도 레포(범용=공용이라 분리 후보). → §10.

---

## 10. 설치기 코드 위치 (메모)

엔진이 **범용(공용)**이므로 장기적으로는 포로 서버들이 공유하는 위치가 맞다. 단기(0.1)는 포로몬에서 시작:
- 단기: `porong-mon/installer/`(엔진+포로몬 pack 빌드 스크립트). 번들 소스=`modpack/client/mods`·`overrides`·PoroMonCore 빌드.
...
- PoroMonCore 빌드: `../../custom-mods/poromon-core/`
