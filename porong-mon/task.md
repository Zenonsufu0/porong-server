...
- ✅ **밸런스 패스 1차(2026-06-07, 결정 035)**: 2단계 가중(stage/tier)+B 고정 cap(최상급≤10%) 코드/데이터 반영. 희귀 70/20/10·컨셉 55/35/10. 최상급 시그니처★20/부수5. 심해·빛·용왕 후보 보강(레지드래고 90%→18%). 단일 진실=`EncounterConfig.probabilities()`(추첨·표시 일치). 빌드+헤드리스 검증(pools 16, 에러0). 표=`encounter_balance_proposal_v1.md`. **잔여**: 필드이벤트 70/30·apex×2 토글·세부 weight 튜닝·등급별 조우권 텍스처·야생 보상 연동(별개).
