...
## 5. 진행 중 / 미해결 TODO (요약)
- ⚠️ **전설 자연스폰 로직 = 미구현(2026-06-07 확인)**: ① **하급·중급 필드 이벤트(결정 019, 2시간 주기 통제 스폰)** 코드 없음 — `field_event_legendary_pool` 데이터(70/30)만 존재, 주기 스폰 미구현(EncounterService에 `field_event` 잔재 1줄). 현재 전설 입수=조우권(개인 아레나) 단독. ② **Cobblemon 기본 전설 자연 스폰 차단(결정 013)도 미적용** — `enableSpawning:true` + 전설 스폰 제거 datapack/설정 없음 → 바닐라 전설이 야생에 희박하게 뜰 수 있음(별도 갭, datapack으로 스폰풀 제거 필요).
