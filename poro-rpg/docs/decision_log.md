...
### DL-130 (2026-06-05) — 포로서버 중앙제어 디스코드 봇 구조 확정 + 스택 정정(DL-030 SUPERSEDE)
...
### DL-131 (2026-06-05) — 봇 오라클 상시호스팅 + 멀티서버 온보딩(공통 디스코드 인증 → 서버별 약관동의 → 서버별 화이트리스트)
...
**관련:** DL-130(봇 구조 정본·스택), DL-030(구 RPG 온보딩 일부 계승), DL-029(`/연동` 인증코드 체계). 코드 변경 없음 — 설계 결정·docs 정합만.

**근거:** 사용자 확인 (2026-06-05).

---

### DL-132 (2026-06-05) — 카테고리 채널 구조 + 이모지 서버선택(역할기반 가시성) + 운영자 닉네임 변경 (DL-131 후속)

**배경:** DL-131 멀티서버 온보딩의 채널/역할 구현 방식을 구체화. 또한 DL-131에서 미정으로 남긴 "다른 마크 계정으로 플레이하려는 경우"의 닉네임 변경 통제 방식을 확정.
...
3. **카테고리 접근 ≠ 화이트리스트.** 카테고리 접근 역할은 카테고리를 **보이게만** 한다(운영 권한 없음 → 자동 부여 허용, 대원칙 ③). 실제 게임 접속(화이트리스트)은 그 안 약관 채널에서 약관동의 + 인게임 `/연동` 인증을 마쳐야 부여(DL-131 §D). → 역할 정책 대원칙 ⑤ 신설.
4. **확장 규칙.** 새 게임/서버 추가 = `카테고리 1개 + <도메인>접근 역할 + 선택 이모지 + (도메인 모듈)` 한 세트.
5. **닉네임 변경 = 운영자 전용.** 닉네임은 공통 인증 시 1회 입력(DL-131), 플레이어 셀프 변경 금지. 다른 마크 계정 전환은 **운영자 문의 → 운영자 명령**으로만: `/닉네임변경 <유저> <새닉네임> <사유>`(직접 교체) 또는 `/닉네임재입력 <유저>`(본인 재입력 허용 토글). 교체는 화이트리스트 정합(구 닉네임 해제/신 닉네임 재인증) + 운영로그 기록 동반, 게임 서버 API 경유.
