# zenon-economy 데이터 스키마 (구상)

> **[STATUS: 구상 — 엔티티 맵 단계]** 경제 시스템의 영속 데이터 모델. 개요 [`concept.md`](../01-vision/concept.md) · 시스템 상세 [`design.md`](../02-design/design.md)(§N) · 결정 [`decision_log.md`](../decision_log.md)(`DL-E###`).
...
| **village_state**(마을 전역 상태) | 진화단계·세부단계(1~20, 5단계×4·시즌1=12)·시대·인프라수준·해금 슬롯 | 단일 | DL-E051·E093·E134 |
| **power_grid**(전력망 상태) | 총공급·총수요·전기세·발전믹스 비율(정책) | 단일 | DL-E083·E087·E093 |

## 다음
- 엔티티별로 필드·타입·관계를 상세화한다(아래에 §로 추가).
- ~~IPO 상장 재무 요건·자본 건전성 임계는 financials 상세 시 정의(DL-E034 이월).~~ → **항목 확정(DL-E090, spec §5-5):** ① 흑자 지속 ② 자기자본 ≥ 최소 자본금 ③ 영업 기간 ④ 부채비율 ≤ 상한, `financials` 자동 판정(임계 P1).
...
> **상태(state)** 만 여기. 행동/동학(소비·노동·출산·투자 함수, 창발 지표)은 [`population_model.md`](../03-economy/population_model.md)로 분리.
