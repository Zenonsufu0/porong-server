...
- ⬜ **약관/인증 버튼 역할전이 DB화**: 현 `TermsAgreeView`/`_promote`는 `ONBOARDING_SERVERS`(env, poromon만) 기반 → 자동 생성 서버의 active 행(3역할 DB저장)에서 읽도록 전환해야 약관→인증→플레이어가 자동 서버에서 동작. (전역 단일 active라 `get_any_active`로 단순화 가능)
