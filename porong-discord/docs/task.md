...
> **T21 1차 슬라이스 구현(2026-06-08, `modules/server_lifecycle/commands.py` + `core/servers.py`):** 🟢 읽기 `/서버목록`·`/서버정보`, 🟡 `/서버신설`(레지스트리 prep 행 생성까지). ⬜ `/서버시작`·`/서버종료`(상태전이+카테고리 가시성), 카테고리/역할 자동생성(T17), mod_log 적재 = 후속.
