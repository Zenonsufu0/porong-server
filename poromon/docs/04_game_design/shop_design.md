...
**상점 접근 = 하이브리드(확정, 결정 024):** 골드가 PoroMonCore 내부 잔액이라 바닐라 주민 거래(에메랄드) 불가 → 거래 실체는 모두 서버 GUI 트랜잭션. 접근 방식만 둘로 나눈다.
...
- 전부 PoroMonCore가 거래 트랜잭션을 처리(외부 상점/경제 모드 미도입). 전설 제단은 PoroMonCore 필수.
