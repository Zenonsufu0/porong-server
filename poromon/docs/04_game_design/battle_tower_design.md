...
## 3-R. 보상 설계 (10층 체크포인트 + 50층 특별)

> 원칙: **층당 최초 1회만 지급**(재도전 무보상 — `league_season_design.md §3`·`gym_badge_design.md §6`과 동일 결). 체크포인트식 진행이라 도달한 층까지의 미수령 보상은 이어서 정산. 골드 흐름 출처 태그 = **배틀타워**(`economy_design.md §6`). **수치는 예시(운영 지표로 튜닝)** — 앵커: 메가팔찌 20,000 / 전설 기본권 15,000(`economy_design.md §9`).
...
### 3-R.2 보스/체크포인트 추가 보너스 (10층 단위 + 보스층)
| 층 | 연출 | 추가 보상(초회) |
|---|---|---|
| 10 | 1차 벽 | +1,000 골드 |
| 20 | 첫 메가 | +2,000 골드 + 기본 메가스톤 택1 후보 |
...
### 3-R.4 1회 완주 총량 (예시 합산)
- 층당 64,000 + 보스 16,000 + 50층 ~15,000 ≈ **약 95,000 골드** + 칭호 + 아이템/조우권 후보.
- 메가팔찌(20,000) 환산 ≈ 4.7개 분량을 **엔드콘텐츠 50층 1회 완주**로 분산 지급 → 파밍 불가(초회 only)라 인플레 위험 낮음. 과다/과소는 `economy_design.md §6` 지표로 튜닝.
- 설정 위치: `rewards.json`/`seasons.json`(향후, `RewardManager`·`EconomyBridge`).
