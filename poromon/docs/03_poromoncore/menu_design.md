...
## 7. 구현 단계
- **0.1**: 리그 패스 지급/복원/보호 + 메인 메뉴 GUI(허브 텔레포트·진행도 조회) — `roadmap.md` Phase 1 완료 기준.
- **0.2**: 매입소·편의 상점 GUI(`EconomyBridge` 연동), 배지/짐 가이드 조회.
- **0.3+**: 허브 시설 안내 + 텔레포트, 야생 귀환, 잠금 모드·커스텀 패스 모델.
