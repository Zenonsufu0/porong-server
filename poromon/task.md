...
- ✅ **legendary_pools.draft.yml 작성**: 16풀 전수에 실 `cobblemon:<id>` + 초기 가중치/enabled. 환상=이벤트 게이트(일반 티어 false, 컨셉/이벤트만 true), apex 시그니처(레쿠쟈/아르세우스/창조신) 저가중. YAML파싱·ID오타·apex동기화 검증 통과. **남은 건 가중치 운영 튜닝 + 희귀 풀(#3)**.
