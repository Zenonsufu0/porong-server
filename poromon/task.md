...
- ✅ **Eggs Addon 동작 검증(결정 027)**: 부화종=`egg/poke/*.mcfunction`의 `spawnpokemon`(loot table은 난수범위만 — 기존 문서 부정확 정정). 기본 풀 common56(스타터+흔한)·rare26(희귀조우권 중복)·shiny81. 알=armor_stand+컴포넌트, 지급=`egg/give/<등급>`. **모드 방랑상인=바닐라화폐 우회→비활성**, 판매는 PoroMonCore 골드 차감 후 give 단일화. 커스텀=mcfunction 오버라이드+loot rolls.max, 가중=인덱스중복.
