...
public final class BattleTowerService {
    ...
    /**
     * 진행 중 타워 배틀 점검(PoroMonCore가 주기 호출): ① NPC 활성 포켓몬이 메가스톤 보유 시
                ...
                for (BattleActor actor : battle.getActors()) {
                    ...
                    for (ActiveBattlePokemon active : actor.getActivePokemon()) {
                        ...
                        if (isNpc && holdsMegaStone(p) && !MegaGimmick.isMega(p)
                            ...
                            PoroMonCore.LOGGER.info("[BattleTower] NPC 메가 발동: {}", p.getSpecies().getName());
                        }
                    }
                }
            } catch (Throwable t) {
                PoroMonCore.LOGGER.error("[BattleTower] tick 처리 실패", t);
            }
            return false;
        });
