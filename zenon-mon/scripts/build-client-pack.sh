#!/usr/bin/env bash
#
# build-client-pack.sh ─ Zenon Mon 클라 배포 팩 빌드 (결정 045 / client_pack_policy.md §6)
# ---------------------------------------------------------------------------
# 목적:
#   1) ZenonMonCore 빌드 → custom-mods/zenon-mon-core/build/libs/zenon-mon-core-*.jar
...
#
# 비고:
#   - manifest의 80개 모드는 CF가 다운로드한다. overrides/mods 에는 ZenonMonCore
...
#   DIST_DIR=/path PACK_NAME=Zenon Mon-0.1 ...     # 출력 경로/이름 덮어쓰기
# ---------------------------------------------------------------------------
set -euo pipefail

# ROOT = zenon-mon 워크스페이스(이 스크립트 상위). 폴더명 변경에 안전.
...
PACK_NAME="${PACK_NAME:-ZenonMon-0.1}"
...
if [ -z "$CORE_JAR" ]; then
  echo "ERROR: zenon-mon-core jar 없음 ($CORE_DIR/build/libs). 먼저 빌드 필요."
  exit 1
fi
echo "  → $CORE_JAR"

# ── [2/5] overrides/mods 번들 (ZenonMonCore 만) ─────────────────────────────
echo "[2/5] overrides/mods 번들..."
mkdir -p "$OVERRIDES/mods"
rm -f "$OVERRIDES/mods/"zenon-mon-core-*.jar       # 옛 버전 잔재 제거
...
# ── [5/5] 요약 ─────────────────────────────────────────────────────────────
echo "[5/5] 완료."
echo "  ZenonMonCore jar : $(basename "$CORE_JAR")"
...
echo "        ② 서버 ZenonMonCore 동기화 — scripts/sync-server-mods.sh (동일 jar 해시)"
