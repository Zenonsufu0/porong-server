...
public final class MenuGuiManager {
    ...
    public static void open(ServerPlayerEntity player) {
        ...
        if (!kr.zenon.moncore.auth.AuthManager.isVerified(player)) {
            kr.zenon.moncore.auth.AuthMenu.open(player);
            return;
        }
        ServerMenuHandler.show(player, Text.literal("Zenon Mon 메뉴").formatted(Formatting.DARK_AQUA),
    ...
    public static void handleClick(ServerPlayerEntity player, int slot, int button, boolean shift) {
        switch (slot) {
            ...
            case SLOT_LEAGUE -> kr.zenon.moncore.league.LeagueMenu.open(player);
            ...
            case SLOT_TRAINING -> kr.zenon.moncore.shop.CategoryShopMenu.open(player,
                    Text.literal("성장 상점").formatted(Formatting.GREEN),
                    ConfigManager.economy().growthShop, "growth");
            case SLOT_TM -> kr.zenon.moncore.shop.CategoryShopMenu.open(player,
                    Text.literal("기술머신 상점").formatted(Formatting.AQUA),
                    ConfigManager.economy().tmShop, "tm");
            case SLOT_ENGINEERING -> kr.zenon.moncore.shop.EngineeringMenu.open(player);
            case SLOT_PRACTICE -> kr.zenon.moncore.shop.CategoryShopMenu.open(player,
                    Text.literal("실전 육성 상점").formatted(Formatting.LIGHT_PURPLE),
                    ConfigManager.economy().trainingShop, "training");
            case SLOT_MEGA_LAB -> kr.zenon.moncore.shop.CategoryShopMenu.open(player,
                    Text.literal("메가 연구소").formatted(Formatting.LIGHT_PURPLE),
                    ConfigManager.economy().megaShop, "mega");
            case SLOT_ALTAR -> kr.zenon.moncore.encounter.AltarMenu.open(player);
    ...
    private static void sendServerGuide(ServerPlayerEntity player) {
        player.closeHandledScreen();
        player.sendMessage(Text.literal("§a§l[Zenon Mon 서버 가이드]"), false);
        ...
        player.sendMessage(Text.literal("§7· 9번 슬롯 §f리그 패스§7 우클릭 = 이 메뉴 (또는 §f/zenonmon menu§7)"), false);
    }
