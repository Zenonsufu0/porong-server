...
import kr.zenon.moncore.config.SeasonConfig;
import kr.zenon.moncore.data.PlayerProgress;
import kr.zenon.moncore.data.ZenonMonState;
import kr.zenon.moncore.encounter.ArenaManager;
...
public final class LeagueManager {
    ...
    public static void joinQueue(ServerPlayerEntity player) {
        ...
        if (QUEUE.containsKey(player.getUuid())) {
            player.sendMessage(Text.literal("§e[정규리그] 이미 큐에서 상대를 찾는 중입니다. §7(/zenonmon league leave 로 취소)"), false);
            return;
        }
        if (!p.rankedInit) {
            p.rankedInit = true;
        ...
        player.sendMessage(Text.literal("§a[정규리그] 큐 참가! 현재 점수 §e" + p.rankedScore
                + "§a. 상대를 찾는 중... §7(취소: /zenonmon league leave)"), false);
    }
    ...
    private static boolean startBattle(MinecraftServer server, UUID aUuid, UUID bUuid) {
        ...
        try {
            ...
            int sa = ZenonMonState.get(server).getOrCreate(aUuid).rankedScore;
            int sb = ZenonMonState.get(server).getOrCreate(bUuid).rankedScore;
            ...
            ArenaManager.clear(arena, corner);
            ArenaManager.free(cell);
            ZenonMonCore.LOGGER.error("[League] 배틀 시작 실패", t);
            return false;
        }
    }
    ...
    private static void onVictory(BattleVictoryEvent event) {
        try {
            ...
            ZenonMonCore.LOGGER.info("[League] {} 승 vs {} 패 ({}→{} / {}→{})",
            ...
            ZenonMonCore.LOGGER.error("[League] onVictory 처리 실패", t);
        }
    }
