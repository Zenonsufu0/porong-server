...
public final class GymBattleService {
    ...
    /** ZenonMonCore.onInitialize 에서 1회 — 승리 이벤트 구독. */
    ...
    public static boolean startChallenge(ServerPlayerEntity player, String gymId) {
        ...
        try {
            ...
            ZenonMonCore.LOGGER.error("[Gym] startChallenge 실패", t);
    ...
    private static void onVictory(BattleVictoryEvent event) {
        try {
            ...
            ZenonMonCore.LOGGER.error("[Gym] onVictory 처리 실패", t);
        }
    }
    ...
    private static void resolve(BattleActor actor, boolean won) {
        ...
        if (firstClear) {
            ...
            ZenonMonCore.LOGGER.info("[Gym] {} 클리어: {} (+{}골드)", player.getGameProfile().getName(), a.gymId, gold);
        } else {
                ...
                for (BattleActor actor : battle.getActors()) {
                    ...
                    for (ActiveBattlePokemon active : actor.getActivePokemon()) {
                        ...
                        if (isNpc && holdsMegaStone(p) && !MegaGimmick.isMega(p) && !MEGA_DONE.contains(p.getUuid())) {
                            ...
                            ZenonMonCore.LOGGER.info("[Gym] NPC 메가 발동: {}", p.getSpecies().getName());
                        }
                    }
                }
            } catch (Throwable t) {
                ZenonMonCore.LOGGER.error("[Gym] tick 메가 처리 실패", t);
            }
            return false;
        });
