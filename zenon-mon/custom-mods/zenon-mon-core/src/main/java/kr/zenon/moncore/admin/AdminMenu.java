...
import kr.zenon.moncore.event.EventManager;
...
public final class AdminMenu {
    ...
    private static void populate(net.minecraft.inventory.Inventory inv) {
        ...
        int baseMin = kr.zenon.moncore.config.ConfigManager.core().fieldEvent.intervalMinutes;
    ...
    private static void promptAnnounce(ServerPlayerEntity player) {
        ...
        kr.zenon.moncore.util.ChatInputManager.await(player, msg -> {
    ...
    private static void releaseAll(net.minecraft.server.MinecraftServer server) {
        for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
            ...
            kr.zenon.moncore.league.LeagueManager.forceEnd(p);
        }
    }
}
