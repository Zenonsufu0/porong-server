...
/**
 * 허브 텔레포트 (menu_design.md §3 슬롯19, commands.md /zenonmon hub).
...
public final class HubManager {
    ...
    public static boolean teleportToHub(ServerPlayerEntity player) {
        ...
        if (!cfg.teleportCommandEnabled) {
            player.sendMessage(Text.literal("§c[Zenon Mon]§r 허브 텔레포트가 비활성화되어 있습니다."), false);
            return false;
        }
        ...
        if (worldId == null) {
            ZenonMonCore.LOGGER.error("[Hub] world id 파싱 실패: {}", cfg.world);
            return false;
        }
        RegistryKey<World> key = RegistryKey.of(RegistryKeys.WORLD, worldId);
        ServerWorld world = server.getWorld(key);
        if (world == null) {
            ZenonMonCore.LOGGER.error("[Hub] 월드 미존재: {}", cfg.world);
            player.sendMessage(Text.literal("§c[Zenon Mon]§r 허브 월드를 찾을 수 없습니다(설정 확인)."), false);
            return false;
        }

        CoreConfig.Spawn s = cfg.spawn;
        player.teleport(world, s.x, s.y, s.z, s.yaw, s.pitch);
        player.sendMessage(Text.literal("[Zenon Mon] 허브로 이동했습니다.").formatted(Formatting.GREEN), false);
        return true;
    }
}
