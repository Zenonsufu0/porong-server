...
>
> **2026-06-05 모노레포 구조 정리:** poromon 자체 폴더/파일은 **변경 없음**. 다만 레포 레벨에서 RPG 자산과 프로젝트 전역 docs가 `zenon-rpg/`로 통합되고 루트 `docs/`가 폐지됨. 포로몬은 계속 `zenon-mon/`(자체 `docs/`·`server/`·`custom-mods/`) 안에서 독립적으로 작업한다.
...
**디스코드 인증 (결정 041, 1인 가능):**
- [ ] 미인증 접속 → 허브 감금(반경 밖 복귀)·메뉴=인증하기만·텔레포트 명령 차단
- [ ] `/인증` → 6자리 코드 발급 → (봇측) 입력 → verify → 인증 완료 안내·감금 해제
- ⚠️ 선행(운영): core.json discordAuth.apiKey 변경 + 봇 측 `/인증코드` 구현(porong-discord, `05_operations/discord_auth_integration.md` 계약)

**챔피언스리그 (결정 040, 2인+ 필수):**
- [ ] `/poromon admin champions start` → 랜덤 대진표 공지 + 1분 카운트다운 + 순차 매치
- [ ] 매치: 두 선수 경기장 TP + 비참가자 관전석 TP + pvp1v1(lvl50) + Cobblemon 관전(인터랙트 휠)
- [ ] 승자 진출·라운드 반복·결승·챔피언 공지 + championHistory 기록
- [ ] 부전승/오프라인 자동 진출·매치 타임아웃·`champions cancel`

**네더 정책 (결정 039-a, 1인 가능):**
- [ ] 오버월드→네더 포탈 = 고정 허브 도착 / 네더→오버월드 = 진입 좌표 복귀
- [ ] 네더 허브 반경 내 블록 파괴 차단(포탈·스포너) + op 우회
- [ ] 네더 월드보더 5000 적용 체감
- [ ] **네더 허브**(결정 039-b/c): `/poromon admin netherhub`(자동 위치) → 플랫폼+벽+귀환포탈+블레이즈 스포너 (op 1회 실행, 헤드리스 자동건설 확인됨)
- [ ] **엔드 정책**(결정 039-d): 입장 시 드래곤 부재 + 바깥섬(엔드시티 권역) 도착 + `/poromon hub` 복귀. 선행: `/poromon admin endhub`(외곽 섬 자동탐색, 헤드리스 (0,57,1100) 확인됨)
...
**이번 세션 작업(결정 042·043 + 운영셋업 + 헤드리스 검증, 커밋 684c60e·0850c92·827bd50·b364bfe):**
- **결정 042 메가스톤/키스톤 월드획득 차단**: MSD 골드독점 우회(레시피180·구조물5·광석2) jar 확인 → `poromon_mega_control` 팩으로 구조물 5종 차단(베이스 재료=구조물 내부 광석뿐이라 공급 100% 차단). 헤드리스 `/locate` "Could not find" 검증.
- **결정 043 OpenLoader 데이터팩 미로딩 발견·수정 (중대)**: 헤드리스 검증 중 OpenLoader 팩이 **하나도 로드 안 됨** 발견 → LM(023)·메가(042)·배틀타워 차단이 **전부 무력**이었음(전설 통제 전제가 깨져 있었음). 원인 ①`options.json load_data_packs=false` ②팩이 `packs/`(인식폴더는 `data/`). 수정 후 재검증: 3팩 enabled + mega/LM `/locate` 전부 차단 확인.
- **운영 셋업(런북 §9 신설)**: apiKey 강한키 생성·런타임 core.json 주입(비커밋, `SPi0-5f_...`) + ping/401/404 엔드포인트 검증. 디스코드 인증 MC측 정상 동작 확인.
- **자투리 4건 확정**: SimpleTMs=동적합성 / Eggs=폐기(032) / 레쿠쟈메가=Dragon Ascent 기술기반·하늘조우권 분리 / config=JSON 통일.
