...
Zenon Mon is a Cobblemon-based Minecraft mod server project.
...
- ZenonMonCore controls server-specific progression, rewards, legendary encounter rooms, unlock rules, and menu systems

This is not the main Zenon RPG server.  
It is a separate modded server line under the Zenon Project brand.

---

## 작업 영역 / 워크스페이스 규칙 (모노레포)

이 저장소는 Zenon 서버 모노레포다. **이 작업 영역(`zenon-mon/`)은 Zenon Mon 모드서버/모드팩만 담당한다.**
...
### 임의 수정 금지 (요청 시에만)
모드 목록 / 조우권 데이터 / Zenon Mon 밸런스(종족값·타입·기술) / 서버 설정값. RPG·디스코드봇 코드.
...
- Added Addons: SimpleTMs (TMs/TRs for Cobblemon), Cobblemon: Legendary Monuments
  ...
  - **Legendary Monuments**: jar 감사 결과 자체 소환(아이템/구조물/바이옴)이 ZenonMonCore 통제를 우회 확정 → **결정 023: 완전 비활성**(worldgen + loot_table datapack 오버라이드로 비활성, 소환 아이템 자연 획득 차단). 전설은 조우권/사설룸으로만. 실제 비활성 datapack은 서버 mods 배치 후 jar worldgen 경로 확인 필요(TODO).
- Development Environment: WSL Ubuntu
- Repo Layout: Zenon 서버 모노레포. Zenon Mon 작업 영역은 모노레포 루트 하위 `zenon-mon/`
  - Zenon Mon 전용 worktree: `/home/zenonsufu1/dev/zenon-work-mon/zenon-mon` (브랜치 `feature/zenonmon-dev`)
...
- Custom Mod Path: `zenon-mon/custom-mods/zenon-mon-core` (Zenon Mon 작업 영역 기준 `custom-mods/zenon-mon-core`)
...
- Zenon Mon server = Fabric mods, server-side mods, configs, datapacks, and custom Fabric mod development
...
ZenonMonCore should only control high-impact progression and server-lifetime systems:
...
ZenonMonCore is the central rules engine of the server.

Cobblemon is the Pokémon engine.  
Mega Showdown is the battle gimmick expansion.  
ZenonMonCore is responsible for Zenon Mon-specific progression and server rules.

ZenonMonCore should eventually include:
...
## Initial ZenonMonCore 0.1 Scope

The first implementation should be small and stable.

Target features:

1. `/zenonmon` base command
...
> 상세 정책·GUI 레이아웃: `docs/03_zenonmoncore/menu_design.md`.
...
- 리그 패스
- Zenon Mon League Pass

Behavior:

- Right-click opens Zenon Mon GUI
- Should not be dropped
- Should not be moved if locked mode is enabled
- Should be restored on join/respawn if missing
- Should have a command alternative: `/zenonmon menu`
...
골드는 ZenonMonCore 내부 잔액(EconomyBridge)이라 바닐라 주민 거래(에메랄드) 불가 → 모든 거래는 서버 검증 GUI 트랜잭션. 접근만 둘로 나눈다:
...
3. ZenonMonCore assigns an empty private encounter room
...
- **하급·중급:** ticket (private room) **+ ZenonMonCore 2-hour field event** (combined low+mid pool 70/30, hidden coords + chat hint, despawn timer, battle grace, one at a time) (decision 019).
...
- **Legendary Monuments는 완전 비활성** (decision 023) — worldgen/loot_table datapack 오버라이드로 소환 경로 차단. 전설은 ZenonMonCore 통제만.
...
ZenonMonCore should control unlock rules and server policy.
...
- Legendary encounter tickets are **ZenonMonCore custom items**, not base-mod items; they open a private encounter room rather than granting a Pokémon directly.
...
Zenon Mon is a Korean-first server. **User-facing text must be in Korean as much as possible** — shop names, item names/descriptions (lore), shop GUI, command feedback.

- ZenonMonCore custom items (e.g., encounter tickets): localize via `ko_kr.json` + Korean lore (owned directly).
...
Gym clear records should be stored by ZenonMonCore.
...
The modpack should be distributed as a single official Zenon Mon pack.
...
2. Install Zenon Mon Official Pack
3. Press Play
4. Join the server
...
- `docs/03_zenonmoncore/zenonmoncore_spec.md`
...
완료 (✔):
- ✔ Zenon Mon 0.1 Dev 모드팩 분석 + 실제 jar 80개 감사 (`docs/01_modpack/jar_feature_audit.md`)
- ✔ 클라/서버 모드 분리 (서버 화이트리스트 19 / 애매 5 / 클라 제외 56)
- ✔ 게임 설계 확정 (decisions 011~022), 조우권·상점·짐·리그 체계
- ✔ 문서 체계 한글화 + 인덱스 (`docs/README.md`)

다음 (Phase 1):
1. **[BLOCKER] Legendary Monuments 처리 방향 결정** — 자체 전설 소환이 ZenonMonCore 통제를
...
이후 (Phase 2+):
6. `custom-mods/zenon-mon-core` Fabric 1.21.1 Gradle 골격
7. `/zenonmon` 명령 → 9번 슬롯 리그 패스 아이템 → 우클릭 메뉴 → config·진행도 저장 스캐폴드
