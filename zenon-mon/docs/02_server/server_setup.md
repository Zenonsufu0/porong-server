...
> 대상: Zenon Mon 데디케이티드 서버 (MC 1.21.1 / Fabric Loader 0.19.3 / Java 21)
> 서버 실행 폴더: **`zenon-mon/.local/server`** (worktree-local, Git 비추적 — RPG DL-130과 동치 표준)
> 모드 분리 기준: `docs/01_modpack/server_mod_separation.md`
> ⚠️ 본 문서는 **계획**이다. 아직 실제 jar 복사/서버 설치는 수행하지 않는다.

## 1. .local/server 디렉터리 구성

서버 실행 폴더는 `zenon-mon/.local/server`이며 `.local/` 전체가 gitignored(런타임 비추적). 골격은 이미 존재(`.local/server/{mods,config,world,logs}` + `eula.txt` + `start.sh`). 각 폴더의 채울 내용:
...
- `server.properties` (포트/난이도/뷰디스턴스/시뮬레이션-디스턴스/online-mode 등)
  - ⚠️ **포트 할당**: Zenon Mon = **25566** (`server-port`/`query.port`). **RPG 서버가 25565를 쓰므로 충돌 회피**. `.local/server`는 비추적이라 재배포 시 이 값 재적용 필요. PvP는 `pvp=false`(결정 011).
...
2. **배치 위치**: `zenon-mon/.local/server/` 바로 아래에 둔다(=start.sh의 작업 디렉터리). 파일명이 바뀌어도 `start.sh`가 `fabric-server*.jar` 글롭으로 잡는다.
...
# ZenonMonCore 개발 전 선행 조건

ZenonMonCore(커스텀 Fabric 모드)는 **서버가 먼저 안정적으로 떠야** 개발/테스트 가치가 있다.
아래 순서를 모두 통과한 뒤에 `custom-mods/zenon-mon-core` Gradle 프로젝트 생성으로 넘어간다.

> ⚠️ 본 단계에서는 ZenonMonCore Gradle 프로젝트를 아직 만들지 않는다.
...
### 단계 4. 클라이언트 접속 확인
- [ ] Zenon Mon 클라 모드팩으로 접속 성공
- [ ] 서버가 클라 전용 모드를 요구하지 않음(모드 불일치 거부 없음)
- [ ] 전투/동기화 정상, 5~10분 TPS 안정

### 단계 5. (이후) ZenonMonCore 빈 모드 로딩 테스트
- [ ] `custom-mods/zenon-mon-core` Gradle 골격 생성 (→ 별도 작업)
- [ ] 빈 `ModInitializer` 빌드 → jar 산출
- [ ] jar를 `.local/server/mods`에 투입 → 서버 기동 시 ZenonMonCore 로드 로그 확인
- [ ] Cobblemon/MSD와 동시 로드 충돌 없음
- [ ] 여기까지 통과해야 `/zenonmon` 등 0.1 스코프 구현 착수
...
- ZenonMonCore 스펙: `docs/03_zenonmoncore/zenonmoncore_spec.md`
