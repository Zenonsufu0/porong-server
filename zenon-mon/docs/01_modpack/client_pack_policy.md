...
> CurseForge 공식 팩 1차 모델은 **자체 제작 범용 exe 설치기 + 전부 번들 + 토글 0.1부터**로 전환됨(사유: CF 로그인·외부 다운로드 차단·유저별 실패). 이 문서는 **CF 방식 참고/패키지 구조 자료**로만 보존. **유지되는 원칙**: 유저 수동설치 금지·T0 서버정합·ZenonMonCore 항상 번들·버전 핀. manifest 정합/`extract-curseforge-pack.sh`/`build-client-pack.sh`(CF zip)는 **CF 전용 → 보류**.

> 대상: **Zenon Mon 0.1** 클라이언트 배포. MC 1.21.1 / Fabric Loader 0.19.3 / Java 21.
...
| ZenonMonCore | `zenon-mon-core-0.1.0.jar` (커스텀) | `custom-mods/zenon-mon-core/build/libs/` |

> **핵심 갭 1 — manifest(80) ≠ 실제(86)**: manifest는 stale 옛 export. 실측(§3-1) = **manifest에 없는 실제 jar 8개**(LM 의존 5 + collection + swingthrough + ZenonMonCore) + **manifest에만 있는 폐기분 1개**(eggs). 8개 중 7개는 CF 재익스포트로 자동 해소, ZenonMonCore만 영구 번들 → §3·§4.
> **핵심 갭 2 — ZenonMonCore는 CurseForge에 존재하지 않음** → 어떤 배포 방식이든 **overrides에 직접 번들**해야 한다(§4).
...
5. **ZenonMonCore는 자체 빌드 산출물** → CurseForge/Modrinth 메타로 해소 불가, 항상 overrides 번들.
...
| 안 | 방식 | 장점 | 단점 |
|---|---|---|---|
| **A. CurseForge 공식 팩** (채택) | CurseForge에 "Zenon Mon Official Pack" 업로드, manifest+overrides | 런처 1클릭 설치·자동 업데이트·유저 친숙 | ZenonMonCore는 CF에 못 올림 → overrides 번들 필요. 승인/심사 절차 |
...
> 사유: 0.1 알파는 **재현 가능한 단일 클라**가 최우선. 토글 UI는 알파 후 수요 확인하고 투자. ZenonMonCore 번들 문제(§4)가 어느 방식에서도 공통 핵심이므로 그것부터 확립한다.
...
└── overrides/               # CF가 해소 못 하는 것 전부(그대로 인스턴스에 복사)
    ├── mods/                # ★ zenon-mon-core + CF에 없는 모드 jar 직접 동봉
...
| `zenon-mon-core` | ❌ 없음(자체) | **(b)** | **overrides/mods 번들** |

> (a) 7개 = LM 의존 5 + collection + swingthrough → 모두 CF 자동해소 후보. (b) 1개 = ZenonMonCore(영구 번들).

**(B) modlist에만 있고 jar 없음 = 1개** → manifest에서 제거
- `eggs-cobblemon-addon` (결정 032 폐기).

**권장 처리 = manifest 재익스포트(가장 안전)**: 현재 CurseForge 프로필에는 이미 (a) 7개가 추가돼 있을 것이므로, **프로필에서 팩을 새로 export**하면 manifest가 자동으로 (a) 전부 포함 + eggs 제거됨. 그러면 남는 건 **ZenonMonCore 번들(b)** 하나뿐. → `extract-curseforge-pack.sh`로 재반영.
...
- `mods/zenon-mon-core-0.1.0.jar`(필수) + (b) 판정 모드들.
...
## 4. ZenonMonCore 번들 — 가장 중요한 제약

ZenonMonCore는 **CurseForge/Modrinth에 없고**(자체 빌드), **클라에도 반드시 필요**(배지·조우권·정수 CustomModelData 텍스처 렌더, `server_mod_separation.md` §1c). 따라서:

1. **빌드**: `cd custom-mods/zenon-mon-core && ./gradlew build` → `build/libs/zenon-mon-core-0.1.0.jar`.
2. **번들**: 그 jar를 `overrides/mods/`에 복사해 팩에 동봉.
3. **버전 동기화**: 서버 `.local/server/mods/`의 ZenonMonCore와 **동일 해시**여야 함(빌드 변경 시 서버+클라+팩 3곳 동시 갱신).
4. **업데이트 흐름**: ZenonMonCore 갱신 = 팩 새 버전 릴리스(유저는 런처에서 팩 업데이트). 커스텀 텍스처/모델 변경도 이 경로.
...
2. **ZenonMonCore 빌드** → `overrides/mods/`로 복사(§4).
...
5. **zip 패키징**: `manifest.json + modlist.html + overrides/` → `Zenon Mon-0.1.zip`(CF 업로드용).
6. **서버 mods 동기화**: `scripts/sync-server-mods.sh`(화이트리스트 25, ZenonMonCore 포함) — 클라 export와 **동일 ZenonMonCore 해시** 확인.
7. **검증 설치**(§7-검증): 깨끗한 런처에 팩 설치 → 서버 접속 성공.

> `scripts/extract-curseforge-pack.sh`·`classify-fabric-mods.py`·`inspect-jars.py` ROOT는 스크립트 위치 기준 동적 도출로 정리됨(옛 `poro-server-poromon` 하드코딩 제거, 결정 048).

---

## 7. 단계별 로드맵

- **0.1 (알파 직전)**: §2-A 단일 풀 프로필. 핵심 = **manifest 정합(§3-1) + ZenonMonCore 번들(§4) + 깨끗한 설치 검증**. 토글 없음.
...
- [ ] 깨끗한 CurseForge 런처에 Zenon Mon 팩 설치 → 모드 86개(또는 정합된 수) 전부 적용
- [ ] ZenonMonCore jar 클라 존재 + 배지/조우권/정수 텍스처 정상 렌더(종이 아님)
...
1. **재배포 라이선스**: overrides에 직접 동봉(b 그룹·ZenonMonCore 외)할 모드의 라이선스가 재배포 허용인가? CF에 있는 건 manifest 참조(다운로드 위임)가 안전 — 가능한 한 (a)로 분류.
2. **JEI vs EMI 택1**(`client_mod_tiers.md` §2-2): 둘 다 동봉 중복 → 운영자 택1?
3. **한글 리소스팩 번들 여부**: 현재 translatable+모드 내장 ko_kr로 충분(task.md §4k). 누락 발견 시 `Zenon Mon-Korean-Pack` overrides 동봉(localization_policy §8.3).
4. **배포 채널 우선순위**: CF 단독 vs CF+Modrinth 동시(packwiz로 단일 소스화 시점).
5. **ZenonMonCore CF 등재 가능성**: 불가(자체 모드) 전제. 영구 overrides 번들 확정.
