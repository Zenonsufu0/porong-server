...
> Zenon Mon은 한국어 유저 중심 서버다. **유저에게 보이는 텍스트는 최대한 한국어**로 제공한다.
...
| 대상 | 방식 | 소유 |
|---|---|---|
| **ZenonMonCore 커스텀 아이템**(조우권 등) | `ko_kr.json` 번역 키 + 아이템 **lore(설명)** 한글 작성 | ZenonMonCore(직접) |
| ZenonMonCore GUI/상점명/버튼/명령 메시지 | 서버측 텍스트를 한국어로(`Text`/Component) | ZenonMonCore(직접) |
...
### 2.1 서버측 vs 클라측
- **서버측 텍스트**(상점 GUI 타이틀, 버튼, 명령 응답, 시스템 메시지): ZenonMonCore가 **한국어 문자열로 직접 전송** → 클라 리소스팩 없이도 한국어 표시.
- **클라측 아이템명/툴팁**(바닐라/모드 아이템 표시): 클라가 자기 언어 파일로 렌더 → **한국어 리소스팩이 클라에 있어야** 한국어로 보임. 서버는 강제 불가 → **공식 팩에 한국어 리소스팩 동봉**으로 해결.
- ZenonMonCore 커스텀 아이템은 `ko_kr.json`을 **모드 jar에 포함**하면 별도 리소스팩 없이도 한국어(클라 언어=ko_kr 시).

## 3. ZenonMonCore 커스텀 아이템 한글화(직접 소유)
- 번역 키 예: `item.zenonmoncore.<id>` → `ko_kr.json`에 한국어명. (실제 키 네이밍은 구현 시 확정)
...
- 공식 한국어 리소스팩(`Zenon Mon-Korean-Pack`)에 각 모드 `ko_kr.json`을 모아 오버라이드.
...
- ✅ 세 코어 모드(Cobblemon/MSD/SimpleTMs) ko_kr 기본 포함 → **한글화 작업량 대폭 감소**(주작업 = LM + ZenonMonCore 커스텀).

## 5. 운영 / 작업 흐름
1. 모드별 `en_us.json`(키 목록) 추출 → 한국어 번역 → `ko_kr.json` 작성.
2. Zenon Mon 한국어 리소스팩에 통합 → 공식 모드팩에 동봉.
3. ZenonMonCore 커스텀 텍스트는 코드/리소스에서 한국어 직접 작성.
4. 검수: 영어 잔존 텍스트(상점/아이템/툴팁) 점검 → 보완.
5. 누락 키는 en_us fallback 허용하되 **유저 핵심 동선(상점/조우권/알/메가)** 은 우선 한글화.

## 5-1. 조우권 / 전설 이벤트 / 상점 GUI 한글화 (ZenonMonCore 소유)
조우권·전설 이벤트는 ZenonMonCore 커스텀이므로 **한국어 직접 작성**(서버측 텍스트 + 커스텀 아이템 ko_kr/lore).
...
4. ZenonMonCore 커스텀 아이템 번역 키 네이밍 규칙 확정.
...
### 8.1 전수조사 결과 (현황 갱신)
- **클라 언어 = `ko_kr`** 확정(Zenon Mon 0.1 Dev `options.txt: lang:ko_kr`).
...
### 8.2 진짜 남은 작업 = ZenonMonCore GUI의 영어 "박제" 제거 (핵심)
...
### 8.3 누락 번역 처리 (필요 시)
- 모드 ko_kr에 빠진 항목(영어로 뜨는 것)만 **자체 `ko_kr.json` 오버라이드**(공식 한국어 리소스팩 `Zenon Mon-Korean-Pack`)로 보완. 키는 해당 모드 `en_us.json`에서 추출.
- 출처: ① 모드 Weblate ko 최신 ② 포켓몬/기술 공식 한글명(Bulbapedia/포켓몬코리아) ③ **사용자 제공 공식 번역본**.
- ZenonMonCore 커스텀(리그패스/정수/배지/조우권/메뉴/메시지)은 **이미 한국어 하드코딩** → 작업 불필요(영어 잔재만 점검).
...
3. (있으면) 누락 모드 번역만 `Zenon Mon-Korean-Pack` ko_kr override 작성 → 모드팩 동봉. **← 다음(클라 캡처로 잔존 확정 후)**
