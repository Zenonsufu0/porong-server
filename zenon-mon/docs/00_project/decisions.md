...
> ⚠️ **배포 모델 부분 폐기 — 결정 046으로 대체(2026-06-12).** "1차=CurseForge 공식 팩 / 토글은 0.2 이후"는 **자체 제작 범용 exe 설치기 + 전부 번들 + 토글 0.1부터**로 전환. CF/Modrinth 배포 배제. **유지되는 것**: "유저 수동 설치 금지·필수(T0) 서버 정합·PoroMonCore는 항상 번들" 원칙. manifest 정합/`extract-curseforge-pack.sh` 등 **CF 전용 산출물은 보류**(설치기 설정 생성으로 대체).
...
### 046. 배포 모델 전환 — 자체 제작 범용 exe 설치기 + 전부 번들 (2026-06-12, 045 대체)

결정 045의 "CurseForge 공식 팩 1차" 배포 모델을 폐기하고 **자체 제작 설치기**로 전환한다. **사유**: CurseForge는 ① 런처 로그인 의존 ② 일부 모드 외부 다운로드 차단 플래그 ③ 유저 환경별 설치 실패가 실측 골칫거리(사용자 경험). 자체 설치기는 토글 UX를 완전 제어하고, 다른 포로 모드 서버(포로건 등)에 재활용 가능.

**결정(사용자 확정 2026-06-12):**
1. **자체 제작 exe 설치기** — Windows `.exe` 더블클릭 배포 우선(Mac 유저 거의 없음 → 후순위/생략). 유저 동선 = exe 실행 → 필수 자동 + 선택 체크 → 설치 → 접속.
2. **범용 엔진 + 서버별 설정 분리** — exe 엔진은 공용, **모드 목록·서버 주소·토글 그룹을 외부 설정(JSON)로 분리**. 포로몬/포로건은 설정 파일만 교체해 같은 엔진 재사용. (CF 공식 팩은 팩마다 CF 등재라 재활용 불가 — 자체 제작의 이점.)
3. **모드 전부 exe 번들** — 모든 jar를 설치기에 동봉. **설치 시 인터넷·로그인 불필요**(외부 플랫폼 의존 0 = "유저별 문제" 최소). PoroMonCore 포함. 트레이드오프: exe 용량↑(~200–400MB), 모드 업데이트 시 exe 재배포.
4. **토글 모델 0.1부터** — 045의 "토글 0.2 이후"를 앞당김. **필수(T0 코어 14)=강제 설치·토글 불가**(서버 레지스트리 정합), **선택(T1 편의/T2 취향)=개별 체크박스**. 분류 출처=`client_mod_tiers.md`.
5. **CurseForge/Modrinth 배포 배제** — 045의 CF 1차·packwiz 소스트리 보류. manifest 정합·`extract-curseforge-pack.sh`·`build-client-pack.sh`(CF zip)는 **CF 전용이라 보류**(설치기 번들 설정 생성으로 대체).

**유지(045에서 살아남는 원칙):** 유저 수동 mods 설치 금지 / T0=서버와 해시 정합·끌 수 없음 / PoroMonCore는 항상 번들(이제 CF overrides가 아니라 exe 번들) / 버전 핀 고정(서버=클라 동일 빌드).
