...
- 전설 조우권은 **ZenonMonCore 커스텀 아이템**(기본 모드 아이템 아님).
...
### 3.5 알 상점 (Eggs - Cobblemon Addon) — ⛔ 폐기(결정 032)
> Eggs Addon/알 시스템 제거(조우권 중복 + 리소스팩 의존). 카테고리에서 알 제외. (아래 보존용)
...
### 3.7 전설 제단 상점 (ZenonMonCore 커스텀)
- 역할: 전설 조우권 판매/사용, 개인 조우방 연동.
- 판매: 일반 5등급(희귀/하급/중급/상급/최상급) + 컨셉별 특수 10종(하늘·심해·대지·시간·공간·반전·빛·용왕·수호자·영원).
- 정책: 조우권 = 포켓몬 직접 지급 X, 제단에서 개인 조우방 입장 후 사용 → ZenonMonCore가 확률표 굴려 소환. 하급 배지 4 / 중급 배지 6 후보 / 상급 배지 8 / 최상급 후반·극고가. **하급·중급은 2시간 필드 이벤트 병행**(상급/최상급은 개인방 중심). 영원(아르세우스) 기본 잠금. **전설 알 판매 금지.** 1인 개인방으로 횡취 방지.
...
## 5. ZenonMonCore 연동 필요 기능
- 배지/관장 클리어 조건 확인, 메가팔찌·메가스톤 구매 가능 여부, 조우권 구매 조건·사용 쿨타임
- 전설 조우 기록, 보상 지급 기록, 구매/사용 로그(`AuditLog`)
- 전설 조우권 = ZenonMonCore 커스텀 아이템 등록·관리

**상점 접근 = 하이브리드(확정, 결정 024):** 골드가 ZenonMonCore 내부 잔액이라 바닐라 주민 거래(에메랄드) 불가 → 거래 실체는 모두 서버 GUI 트랜잭션. 접근 방식만 둘로 나눈다.
- **9번 슬롯 메뉴(리그 패스) GUI — 어디서나**: 일반 상점(볼·회복약) + 매입(광물·농작물·베리). 진행도·배지 조회·허브 텔레포트 포함. 자주 쓰는 환금/소모는 접근성 우선.
- **허브 NPC/판매대 우클릭**: 성장·실전 육성·기술머신·알·메가 연구소·전설 제단. **배지·관장 클리어 게이트 판정은 허브에서** → 통제·해금 상품은 허브 회귀 유도(§1 철학)·위치 몰입.
- 전부 ZenonMonCore가 거래 트랜잭션을 처리(외부 상점/경제 모드 미도입). 전설 제단은 ZenonMonCore 필수.
...
> `roadmap.md` Phase 4~6 / `zenonmoncore_spec.md` 0.1(데이터 모델·명령)과 매핑.
...
- 코어: `../03_zenonmoncore/module_structure.md` / `config_structure.md` / `database_schema.md`
