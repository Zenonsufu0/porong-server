# ZenonMonCore Module Structure (모듈 구조)

> ZenonMonCore = Zenon Mon 서버의 **규칙 엔진**(Cobblemon=포켓몬 엔진, MSD=배틀 기믹).
> 상위: `zenonmoncore_spec.md`(역할·스코프·매니저 목록). 본 문서는 **패키지/모듈/의존/초기화 순서**를 고정한다.
> 베이스 패키지: **`kr.zenon.moncore`** (CLAUDE.md). Java 21 / Fabric 1.21.1.
...
```
kr.zenon.moncore
├─ ZenonMonCore.java          # ModInitializer (서버/공통 진입점, 매니저 부트스트랩)
├─ ZenonMonCoreClient.java    # ClientModInitializer (클라 전용; src/client)  ※ 0.1에선 최소
│
...
├─ command/     AdminCommandManager + /zenonmon 커맨드 트리
├─ util/        공통 유틸(좌표/텍스트/권한/로깅) + i18n/LocalizationManager(한국어 텍스트)
└─ client/      ZenonMonCoreClient 하위 — 화면/렌더/키바인드 (클라 전용)
```
...
| AdminCommandManager | command | 서버 | ✅(reload) | `/zenonmon` 관리자 하위명령 |
...
부트스트랩(서버 시작 시 `ZenonMonCore.onInitialize`):
```
...
```jsonc
"entrypoints": {
  "main":   ["kr.zenon.moncore.ZenonMonCore"],          // 서버+공통
  "client": ["kr.zenon.moncore.client.ZenonMonCoreClient"]
}
```
...
| 0.1 기능(spec) | 담당 모듈 |
|---|---|
| `/zenonmon` 기본 명령 | command(AdminCommandManager) |
