...
> 설정: `config_structure.md §3 core.json`(`menuItem`/`hub`) · 명령: `commands.md`(`/zenonmon menu`, `admin pass`).
...
| `displayName` | `리그 패스` | 표시명(한국어). en_us fallback `Zenon Mon League Pass` |
...
**식별:** 아이템 NBT/Component에 ZenonMonCore 태그(예: `zenonmoncore:league_pass=1`)로 식별 → 일반 시계와 구분, 복제/위조 방지. 인벤토리 정리 시 1개 초과분은 회수.
...
- 명령 대안 `/zenonmon menu`(패스 분실/비활성 대비) — `commands.md`.
- 관리 `/zenonmon admin pass <player>` 재지급.
...
> ⚠️ **결정 030(허브 단순화)으로 변경:** 핵심 sink 상점(37–41)은 **메뉴 GUI에서 직접 거래**한다. (결정 024의 "허브 NPC에서만 거래"는 폐기 → 허브엔 NPC/판매대를 두지 않고 모든 거래·해금·조우를 메뉴로 통합. 37–41 = 해당 카테고리 상점 GUI 입구.)
...
### 4.3 핵심 sink 상점 (메뉴 직접 거래 — 결정 030)
- 각 카테고리(전설 제단·메가 연구소·기술머신·알·육성)를 클릭하면 **해당 상점 GUI를 직접 연다.**
- 매매·해금 판정(배지·관장 게이트 포함)을 **메뉴 GUI에서 서버 검증 트랜잭션**으로 처리. 허브 NPC/텔레포트 안내 아님(결정 030으로 하이브리드 폐기).
