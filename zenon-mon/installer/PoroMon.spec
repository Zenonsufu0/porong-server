...
# PoroMon 설치기 PyInstaller spec (결정 046 / installer_design.md §6)
#
# 선행: ../scripts/build-installer-pack.sh 로 번들 생성
#       (.local/installer-pack/PoroMon = pack.json + mods/ + overrides/ + tools/)
# 빌드(Windows 권장 — exe 타겟은 Windows에서):
#       cd installer && pyinstaller PoroMon.spec
#       → dist/PoroMon설치기.exe (번들 동봉, 단일 파일)
#
# SPECPATH = 이 spec 파일이 있는 디렉터리(installer/). PyInstaller 가 주입.
import os

bundle_src = os.path.join(SPECPATH, "..", ".local", "installer-pack", "PoroMon")
...
a = Analysis(
    ...
    hiddenimports=[
        "poromon_installer.pack",
        "poromon_installer.nbt",
        "poromon_installer.steps",
        "poromon_installer.installer",
        "poromon_installer.platform",
    ],
    hookspath=[],
    runtime_hooks=[],
    excludes=[],
