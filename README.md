...
> 🤍 **YUKI-01 / 유키** — Zenon Project의 공식 마스코트이자 Discord 비서봇 캐릭터(흰색+하늘색+눈송이 테마의 SD 메이드 안드로이드 비서). 운영 봇(`zenon-discord`)의 페르소나로 사용한다.
...
- 현재는 **RPG가 주력 개발**, `zenon-mon`/Discord 봇이 병행, 나머지는 **구상 단계** 후보 서버다.
...
| 프로젝트 | 폴더 | 컨셉 |
|---|---|---|
| **zenon-economy** | [`zenon-economy/`](zenon-economy/) | **경제 특화 후보 서버.** 유저 간 거래·시장·생산·경제 밸런스가 핵심 루프. 플랫폼·규모 미확정. |
| **zenon-gun** | [`zenon-gun/`](zenon-gun/) | **총기/전술/생존 후보 서버.** 후보 방향: 타르코프식 익스트랙션 · 배그식 배틀로얄 · 좀비 서바이벌 · 총기 PvE/PvP. **세부 컨셉 미확정.** |
...
| 단계 | 프로젝트 | 의미 |
|---|---|---|
| 활성 개발 | zenon-rpg, zenon-mon, zenon-discord | feature 브랜치 + 전용 worktree에서 실제 개발/실행 중 |
| 구상 | zenon-economy, zenon-gun | 컨셉/아이디어 수집 단계. 코드·런타임 없음, 문서만 추적 |

---

## 🧭 전역 규칙 (요약)

- **설계 단일 진실 공급원(SoT):** [`zenon-rpg/docs/final_master_plan.md`](zenon-rpg/docs/final_master_plan.md). 충돌 시 최우선. 도메인 세부는 각 프로젝트 docs의 `CANON.md`.
- **결정 기록:** 설계/방향 결정은 [`zenon-rpg/docs/decision_log.md`](zenon-rpg/docs/decision_log.md)(DL-NNN), 미확정은 각 프로젝트 `docs/idea_inbox.md`.
...
| `zenon-work-rpg` | `feature/rpg-dev` | `zenon-rpg` | 활성 |
| `zenon-work-mon` | `feature/poromon-dev` | `zenon-mon` | 활성 |
| `zenon-work-discord` | `feature/discord-dev` | `zenon-discord` `zenon-rpg/docs` `zenon-mon/docs` | 활성 |
| `zenon-work-economy` *(예정)* | `feature/economy-dev` | `zenon-economy` | 구상 — 착수 시 생성 |
| `zenon-work-gun` *(예정)* | `feature/gun-dev` | `zenon-gun` | 구상 — 착수 시 생성 |

- **in-repo 폴더 이름(`zenon-rpg/` 등)은 이번 1차 브랜드 전환에서 그대로 유지한다.** SoT 경로·문서·`.gitignore`가 참조하므로 함부로 rename하지 않는다. worktree **디렉토리** 이름(`zenon-work-*`)은 별개 레이어다.
