...
```
/버그제보 → 대상 선택(RPG / Zenon Mon / 기타·봇) → 모달(제목·재현·기대/실제·심각도)
```
...
- **라우팅(경계 유지 — 게임 버그는 게임 DB):**
  ...
  - Zenon Mon: `zenon_mon_api`(T4) 동형 — Zenon Mon 게임 DB + Zenon Mon 채널. **API 확정 선행**.
...
- 운영 관리: `/faq추가`(대상=공통/RPG/Zenon Mon, 모달)·`/faq수정 번호`(모달 프리필)·`/faq삭제 번호`(admin·support) → `faq` CRUD. 번호 = 자동완성(`#id [도메인] 질문`). 전부 `mod_log`(faq_add/update/delete).
...
- Zenon Mon 버그 API 스키마(ZenonMonCore 선행).
</content>
