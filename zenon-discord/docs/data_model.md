...
- 🟢 **v6(2026-06-09): 전역 active 1개 강제** — `idx_servers_single_active`(UNIQUE ON `state` WHERE `state='active'`). RPG·Zenon Mon 통틀어 항상 한 서버만 active(task.md §5 전역 단일 모델). 기존 per-domain 인덱스는 포섭됨.
