...
여러 게임 서버(RPG·Zenon Mon)의 정보를 통합 보유하고, 역할 부여와 알림을 담당한다.

- 실제 게임 로직은 RPG 플러그인 / Zenon Mon 서버가 처리한다.
- 봇은 **명령어 · 권한 · 알림 · 조회 · 운영자 UI** 만 담당한다.
- RPG/Zenon Mon은 코드 공유가 거의 없으므로 **봇 내부에서도 도메인을 격리**한다.
...
| [`domains/poromon.md`](domains/poromon.md) | Zenon Mon 도메인 (스텁/TODO) |
...
| 멀티서버 온보딩(공통 디스코드 인증 → 서버별 약관동의 → 서버별 화이트리스트, DL-131) | 🟡 RPG만 구현, 공통 분리·Zenon Mon 설계 |
| 배포(오라클 상시 운영, 게임 호스팅 분리, DL-131) | 🔴 미구성 |
| Zenon Mon 연동·명령어 | 🟡 스텁(인터페이스/TODO) |
