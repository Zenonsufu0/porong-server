# 포롱서버 디스코드 봇 — 작업 목록 (task.md)

> **[STATUS: ACTIVE]** — porong-discord 구현 작업 트래커.
> 구조 정본은 [`architecture.md`], 통신 계약은 [`integration_contract.md`], 결정 근거는 `../../porong-rpg/docs/decision_log.md` **DL-130~134**.
...
### rpg/ 🟢 (player_commands · field_boss)
- 🟢 프로필/영지/보스 조회, 필드보스 알림 구현.
- ❌ **auth.py·role_poll.py 폐기(2026-06-09, DL-138):** 구방향(봇 발급→인게임 `/연동`, 닉 모달·role-queue 폴링) 제거. 온보딩은 `modules/onboarding`(신방향 verify)로 통일. `rpg_api` 구방향 4메서드도 제거.
...
> **✅ 재설계 확정 (2026-06-09): 전역 active 서버 1개 → 서버선택 제거.** RPG·포로몬 통틀어 **항상 한 서버만 운영**(동시 시즌·동시 게임 미운영). "domain당 active 1"을 **전역 active 1**로 강화(SUPERSEDE). 들어오면 진행 중 서버를 자동 배정 → 선택 단계(T10) 폐기.

표준 흐름 (전역 단일 active 서버 전제 — 채널 가시성 = 역할 권한 오버라이트):
1. 포롱 디스코드 서버 가입 → **[자동] 진행 중 서버의 `<서버>임시`(=접근) 역할 부여**(active 없으면 `서버준비`).
2. **온보딩 카테고리(`<표시명> · 임시`)**: 임시역할 = **약관 채널만** 보임(읽기전용).
3. 약관 채널의 **약관동의 버튼** → `임시` → **`<서버>인증전`** 전환 → **인증 채널만** 보임(약관 숨김). 인증은 약관 동의 후에만.
4. 인증 채널의 **🔑 인증코드 입력 버튼 → 모달** → 봇 verify (인게임 `/인증` 발급 코드).
5. 인증 성공 → `인증전` → **`<서버>`(=플레이어)** → **온보딩 카테고리 사라지고** 나머지 카테고리(정보·커뮤니티·지원·음성) + 통합 보임.

**생애주기 일괄 역할 전이 (전역 단일 active 전제):**
- `/서버종료` → 모든 멤버의 해당 서버 역할(임시·인증전·플레이어) **일괄 회수** + **`서버준비` 일괄 부여**. `서버준비` = **통합 카테고리(공지·채팅)만** 보임.
- `/서버시작` → **`서버준비` 일괄 회수** + 시작한 시즌의 **`임시` 역할 일괄 부여** → 다시 온보딩 시작.
> `서버준비`(전역 역할)·`통합 카테고리`(공지·채팅, 항상 존재)는 운영자 사전 구성(`.env`: `ROLE_서버준비_ID`·`CATEGORY_통합_ID`). 미설정 시 일괄 전이 graceful skip.
> ⚠ 일괄 부여/회수 = 전체 멤버 순회(members 인텐트) → 인원 많으면 수십초~분(디스코드 레이트리밋). 명령은 defer 후 처리·완료 보고.
> ⚠ DL 동기화 대기(전역 active 1 = 설계 결정, RPG worktree에서 DL 부여 — 디스코드 워크트리는 decision_log 읽기전용).
> 기존 `미인증→접속대기→인증유저`의 서버별 버전. T21 레지스트리·3역할(`servers.access/pending/player_role_id`) 연계.
...
### 배포 — 상시 운영 (호스팅 위치 무관)
- 🟡 **T8. 상시 호스팅(24h, 게임 호스팅과 분리, DL-131).** 런북 = [`deployment.md`](deployment.md)(2026-06-10).
  - 🟢 배포 문서/체크리스트: 인텐트(members ON·message_content OFF)·봇 권한(Manage Channels/Roles·Moderate/Kick/Ban·Move)·.env 분류(필수 4 + 선택)·기동·운영 런북·스테이징 e2e 체크리스트. `.env.example` 누락 10키(운영권한·알림역할) 보강.
  - ⬜ 실제 인스턴스 프로비저닝 + 프로세스 관리(systemd/docker) + 봇→게임 API 도달성(방화벽). ※ 위치는 오라클 비필수(어디든 Python 3.12+.env).
  - ⬜ 스테이징 실길드 e2e 수행(체크리스트 소진).
...
### 📌 세션 종합 (2026-06-09, feature/discord-dev) — 구현 대형 패스 (19커밋, e710da9..083aed3, 전부 push됨)
설계만이던 영역을 대거 **실코드로 구현**. 운영/온보딩/생애주기/커뮤니티/지원의 핵심이 동작 수준에 도달.

**완료(이번 세션):**
- **운영/감사:** mod_log 인프라(v2) + `#운영로그` / 모더레이션 T15(경고계·타임아웃·추방·차단·대상보호).
- **서버 생애주기/온보딩(핵심 재설계):** T17 프리픽스 카테고리 4그룹+온보딩 3역할 자동 전개(v4) · 약관 DB 저장(v5, `/약관설정`) · **전역 단일 active 모델**(v6, 서버선택 폐기·입장 자동배정·생애주기 일괄 역할전이·통합 카테고리) · 온보딩 패널 DB화(`get_any_active`) · `/서버시작` 패널 자동게시 · RPG verify 클라이언트 추가 · **RPG 구방향 auth 폐기**(DL-138).
- **공통/커뮤니티/지원:** 접속정보 라이브(v7, SLP/mcstatus) · 임시 음성방(T13) · 커뮤니티 레벨+칭호+XP보정(v8·v9, T13 코어 완료) · 티켓 1:1 문의(v10, T16 일부).
- **DB 마이그레이션 누적:** v1(servers)→v10(tickets). schema_meta 증분 러너, append-only.

**확정 결정(DL 동기화 대기 — RPG worktree, 디스코드 워크트리는 decision_log 읽기전용):**
...
**검증:** compileall + stdlib sqlite v1~v6 + terms upsert + 전역 active 1 강제. 실길드 e2e(역할/카테고리/모달/패널/일괄전이)는 스테이징.
...
### 재진입 메모
- 작업 worktree: `porong-work-discord`(디스코드 전용). 브랜치 `feature/discord-dev`.
- 문서 진입점: [`index.md`] → architecture / integration_contract / roles_and_permissions / notifications / domains/*.
- DL 로그: `../../porong-rpg/docs/decision_log.md` DL-130~134.
...
> 이 섹션은 승격 항목의 **상세 명세·근거** 보관처. DL(`../../porong-rpg/docs/decision_log.md`) 기록은 RPG worktree에서 동기화 예정(여기선 읽기전용).
...
| 접속정보 게시 🟢 | active 서버 `접속정보` 채널에 접속주소·on/off·인원·핑을 임베드로 게시·3분 주기 갱신(2026-06-09 구현). **SLP(mcstatus)로 직접 핑** → 게임서버 커스텀 API 불필요(RPG·포로몬 공용). 주소=`servers.connect_address`(`/서버주소`·신설 옵션). 메시지 ID 영속(status_message_id) | 2 |
