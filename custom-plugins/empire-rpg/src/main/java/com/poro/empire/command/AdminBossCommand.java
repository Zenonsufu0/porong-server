 ...
 * <ul>
 *   <li>/empire-boss-list — 진행 중 보스 런 목록</li>
 *   <li>/empire-boss-end &lt;runId&gt; — 강제 종료 (runId 접두어 매칭)</li>
 * </ul>
 */
public final class AdminBossCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (command.getName().equalsIgnoreCase("empire-boss-end")) {
    ...
    private boolean handleList(CommandSender sender) {
        ...
        if (!runs.isEmpty()) sender.sendMessage("§8강제 종료: /empire-boss-end <runId 앞 8자리>");
        return true;
    }

    private boolean handleEnd(CommandSender sender, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§c사용법: /empire-boss-end <runId 앞 8자리>");
            return true;
        }
        ...
        if (result.isFailure()) {
            ...
            Bukkit.getLogger().info("[empire-boss] " + sender.getName() + " force-ended run " + runId);
        }
        return true;
    }
