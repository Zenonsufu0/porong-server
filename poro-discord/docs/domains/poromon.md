...
- **PoroMonCore(커스텀 모드)가 RPG와 동일한 HTTP API 패턴을 노출**한다. `poromon_api` 가 그 클라이언트.
- 봇 관여 경계는 RPG와 동일 — 게임 상태 권위는 포로몬 서버, 봇은 API 클라이언트(DL-133).
- 이벤트 알림은 **포로몬 서버 → 봇 push**(공유 시크릿/HMAC 보호) → `core/notifier` 게시.
- 온보딩은 멀티서버 모델을 따른다 — 포로몬 카테고리(이모지 `포로몬접근`) → 포로몬 약관동의 →
  포로몬 `/연동` → 포로몬 화이트리스트 + `포로몬플레이어`([`../roles_and_permissions.md`] §A-2·§D).
...
- 연결 방식은 HTTP API로 확정(DL-133). 남은 것: **API 포트·인증 시크릿·엔드포인트 계약**
  (인증/조회/이벤트 push 스키마)과 조회 가능 데이터 범위.
  PoroMonCore(`../../poromon/docs/03_poromoncore/`) 설계 확정과 함께 결정.
