# poro-discord — 포로서버 중앙제어 디스코드 봇 작업 지침

> 이 문서는 `poro-discord/` 작업 영역 전용 지침이다.
...
- 실제 게임 로직은 RPG 플러그인(`../poro-rpg/`) 또는 포로몬 서버(`../poromon/`)에서 처리한다.
...
수정 가능:
- `poro-discord/` 내부 전체 (이 폴더)
- (분리돼 있다면) 디스코드봇 전용 문서

읽기만 가능 (수정 금지):
- `../poro-rpg/`, `../custom-plugins/poro-rpg/`, `../poromon/`
...
### 봇이 요구하는 환경변수 (`.env`)
필수: `DISCORD_TOKEN`, `PORO_API_KEY`, `GUILD_ID`, `CHANNEL_AUTH_ID`, `CHANNEL_FIELD_BOSS_ID`,
`ROLE_접속대기_ID`, `ROLE_인증유저_ID`
선택(미설정 시 0 → 기능 비활성): `PORO_API_URL`, `TERMS_MESSAGE_ID`, `ROLE_미인증_ID`,
