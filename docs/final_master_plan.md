...
- 현재 기준은 이 문서, 각 도메인 `CANON.md`, `docs/decision_log.md`의 최신 DL 항목 순서로 확정한다.
- 상세 스펙 문서가 CANON과 충돌하면 상세 스펙 문서를 수정하거나 archive로 이동한다. CANON을 구버전 상세에 맞춰 되돌리지 않는다.
- `docs/_archive/` 문서는 과거 맥락 추적용이며 현재 구현·수치·GUI 기준으로 사용하지 않는다.
...
| 항목 | 상태 | 담당 문서 |
|---|---|---|
| 강화석 드랍률 최종 수치 | **확정 (DL-033)** — 필드1 6% / 필드2 8% / 필드3 9% / 필드4 10% / 필드5 12% | `docs/06_fields_bosses/drop_tables_v1.md` |
| 11~16강 강화 성공률 하향 | **확정 (DL-033)** — 11강 25% / 12강 20% / 13강 17% / 14강 15% / 15강 12% / 16강 10% | `docs/02_database_api_stats/economy_numbers_v2.md` |
| 21~25강 강화 성공률 | 현행 유지 확정 (DL-014 이후 블로커 해소) | `docs/02_database_api_stats/economy_numbers_v2.md` |
