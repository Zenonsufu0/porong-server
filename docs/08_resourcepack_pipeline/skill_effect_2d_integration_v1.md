...
- **운반 아이템(carrier)**: 플레이어에게 주지 않는 디스플레이 전용 아이템 1종으로 통일 추천 — `minecraft:paper`(이미 `items/paper.json` 존재) 또는 전용 미사용 아이템. cmd 400xxx 케이스만 추가.
...
- carrier 아이템 선택(`paper` 재사용 vs 전용).
...
- 보스 패턴 hook: `boss_pattern_mythic_mapping`, `docs/07_boss_pattern_modules/`.
