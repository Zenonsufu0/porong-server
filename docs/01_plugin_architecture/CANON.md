...
> **[STATUS: CANON]** — 플러그인 경계, EmpireRPG 책임 범위, 부트스트랩 순서, 운영 명령어의 공식 기준.
...
|---|---|
| 코어 플러그인 | EmpireRPG |
| EmpireRPG 책임 | 전투, 장비, 영지, 보스, 보상, DB, API, Discord/web 연동 데이터 |
...
| 문서 | 역할 |
|---|---|
| `../final_master_plan.md` §4, §11, §15 | 전체 플러그인 방향, API, `/empire` 관리자 커맨드 요약 |
| `implementation_reference.md` | DB 스키마, 이벤트 훅, 클래스 배정 등 구현 상세 |
| `admin_command_spec.md` | `/empire` 서브커맨드 상세 |
| `index.md` | 플러그인 운용 구조 개요 |
| `empire_rpg_design_intent.md` | EmpireRPG 설계 의도 |
| `empire_rpg_module_design.md` | 모듈 단위 설계 참조 |
...
이 폴더의 다른 문서가 Citizens 사용, NPC 기반 진행, 또는 EmpireRPG 외부 플러그인이 핵심 데이터를 소유하는 구조를 전제하면 이 문서와 `../final_master_plan.md`를 우선한다.
