...
| 스폰 경로 | `FieldSpawnService` → `mythicSpawner` BiFunction(EmpireRPGPlugin:642~). 람다가 스폰 **Entity** 보유 | **스폰 직후 어트리뷰트 주입 지점 존재** |
| 몹 식별 | scoreboard tag(`empire_field_N`/`empire_rank_elite`/`empire_type_field_boss`) + mobId | 오버라이드 키로 사용 |
...
- 대안 (택1, 후속 결정):
  - **(C-1) 글로벌 배율 placeholder** — MythicMobs 스킬을 `damage{a=<배율변수>×기본}` 형태로 바꾸고, EmpireRPG가 PlaceholderAPI/Mythic 변수로 보스별 패턴 배율을 노출. 런타임 명령으로 그 변수만 조정. (가장 유연)
  - **(C-2) YAML 배포 유지** — 패턴 데미지는 DL-116 §6대로 YAML 배포로 두고, 런타임은 HP+평타만. (MVP 단순)
- **권고:** MVP는 (C-2). 패턴 배율 핫에딧 수요가 확인되면 (C-1) 도입.

**명령:** `/empire-mobstat <mob_key> <hp|def|atk> <value>` · `/empire-mobstat list` · `/empire-mobstat reset <mob_key>`
...
**명령:** `/empire-shop add <category> <item_id> <price> [amount]` · `/empire-shop setprice <item_id> <price>` · `/empire-shop remove <item_id>` · `/empire-shop reload`
...
- 운영자 permission(`empire.admin.config`) 또는 op 게이트.
- 값 클램프: HP/ATK 음수·과대값 거부(예: ATK 0~1000, HP 1~100000).
- 롤백: `config_change_log` 기반 `/empire-config undo <id>` (old_value 복원).
