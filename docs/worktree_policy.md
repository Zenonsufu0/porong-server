...
> **브랜드 메모(2026-06-13, DL-139):** 레포/worktree **표시명**은 Zenon Project 기준(`zenon-server`/`zenon-work-*`)으로 1차 전환했다.
> **in-repo 폴더명(`porong-*`)은 이번 1차 전환에서 그대로 유지한다** — SoT 경로·`.gitignore`·다수 문서가 참조하므로 rename은 별도 후속 작업으로 보류. 아래 본문도 이 기준을 따른다.

> Zenon Server monorepo는 **하나의 합본 저장소**를 두고, 프로젝트별 개발/실행은
...
3. merge는 원본 `zenon-server`(master)에서 한다.
...
`git -C /home/zenonsufu1/dev/zenon-server worktree list` 결과:
...
> **GitHub repo 이름**도 `poro_server` → `porong-server`로 변경했었고, origin remote URL을
