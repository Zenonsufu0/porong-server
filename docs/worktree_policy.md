...
|---|---|---|---|
| `/home/zenonsufu1/dev/poro-server` | **합본/merge/기록용** 원본. 전체 구조 확인. | ❌ 안 함 | ❌ 안 엶 |
| `/home/zenonsufu1/dev/poro-work-rpg` | RPG 개발/실행 worktree | ✅ Paper RPG | `poro-work-rpg/poro-rpg` |
| `/home/zenonsufu1/dev/poro-work-poromon` | PoroMon 개발/실행 worktree | ✅ Fabric/Cobblemon | `poro-work-poromon/poromon` |
| `/home/zenonsufu1/dev/poro-work-discord` | Discord 봇 개발/실행 worktree | ✅ Discord 봇 | `poro-work-discord/poro-discord` |
...
> **worktree 디렉토리 이름**(`poro-work-rpg` 등)과 **in-repo 폴더 이름**(`poro-rpg` 등)은
...
```bash
# RPG
cd /home/zenonsufu1/dev/poro-work-rpg
git sparse-checkout init --cone
git sparse-checkout set poro-rpg

# PoroMon
cd /home/zenonsufu1/dev/poro-work-poromon
git sparse-checkout init --cone
git sparse-checkout set poromon

# Discord (봇이 RPG/PoroMon 문서를 참조하므로 docs 하위만 추가 포함)
cd /home/zenonsufu1/dev/poro-work-discord
git sparse-checkout init --cone
...
|---|---|---|
| `poro-work-rpg` | `poro-rpg` | |
| `poro-work-poromon` | `poromon` | |
| `poro-work-discord` | `poro-discord` `poro-rpg/docs` `poromon/docs` | docs는 참조 전용. RPG/PoroMon 코드는 안 가져옴 |
...
3. merge는 원본 `poro-server`(master)에서 한다.
...
2. 새 worktree를 만든다:
   `git worktree add /home/zenonsufu1/dev/poro-work-foo <branch>`
...
`git -C /home/zenonsufu1/dev/poro-server worktree list` 결과:

```
poro-server          [master]              ← 합본/merge/기록
poro-work-rpg        [feature/rpg-dev]     ← sparse: poro-rpg
poro-work-poromon    [feature/poromon-dev] ← sparse: poromon
poro-work-discord    [feature/discord-dev] ← sparse: poro-discord poro-rpg/docs poromon/docs
poro-server-review   [codex-review]        ← 코드리뷰 전용(별도 운영)
```
