...
> **[STATUS: REFERENCE]** — 현재 EmpireRPG 코드가 기획서·CANON 기준과 얼마나 일치하는지 점검하고, 실행되지 않거나 화면에 보이지 않는 기능의 원인을 정리하는 구현 계획서.
>
> 작성일: 2026-05-28
...
현재 `custom-plugins/empire-rpg`에는 여러 도메인 클래스와 GUI, 보스, 보상, 저장 로직이 존재하지만 실제 서버에서 실행되지 않거나 사용자에게 보이지 않는 기능이 많다.
...
| 항목 | 문서 기준 | 현재 구현 상태 | 판정 |
|---|---|---|---|
| JSON 위치 | `plugins/EmpireRPG/playerdata/{uuid}.json` | 동일 | 일치 |
...
| 2 | `EmpireRPGPlugin`의 `NpcSyncBootstrap` 호출 제거 또는 비활성화 | 부트스트랩 순서 정합 |
...
| 7 | `empire_field_1` 일반몹 처치 | 필드1 일반몹 보상 |
| 8 | `empire_field_1`, `empire_rank_elite` 정예몹 처치 | 정예 보상과 흔적 확률 적용 |
...
| 컴파일 | `cd custom-plugins/empire-rpg && bash ./gradlew compileJava` |
