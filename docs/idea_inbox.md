...
- 항목:
  ...
  - **마법사 자원 5단계** — ℹ️ **정본 의도** (`combat_balance_v2 §4`: 스태프·창만 5단계, 나머지 4무기 3단계). `HealthHudFormatter:211`·`GrowthGuiListener:2206` `(SPEAR||STAFF)?5:3`. 버그 아님. 3으로 변경 시 스태프 "충전형 폭발" 정체성 상실 + 정본 변경 필요 → 사용자 최종 결정 대기.
...
- 상태: DRAFT (원거리 평타 구현 / 자원 5·피드백 메시지는 사용자 결정 대기)
