...
## 1. 기술 스택
- **Forge 1.20.1** (dev 서버 47.4.10) / **Java 17 target**(빌드) — dev 구동은 Java 21(호환). ❓ *빌드 target 17 확정? (1.20.1 표준)*
...
## 2. 데이터 영속 ❓ (착수 핵심 결정)
저장 대상: 코어/기지 레벨·영역·**블록 내구**·화폐·인증 매핑·보안칸 내용·연합(OPaC?)·신규 보호막 타이머·통계.
- **❓ 저장 방식 분리:** ①게임 상태(코어·블록 내구·화폐·보안칸) = 월드 `SavedData`(NBT)? ②인증·통계·계정 연결 = **외부 DB**(`data-schema` 에이전트가 웹/봇 조회용 DB 설계 — 그것과 연동?) ③config = 수치 튜닝(datapack/toml)?
- **❓ 블록 내구 저장 방식:** base_raid v2가 "좌표별 내구 데이터 **또는** 영역 일괄 배율" 둘 다 언급 → **어느 쪽?**(좌표별=정밀·메모리↑ / 영역 배율=단순·블록별 손상 추적 약함)
...
1. **빌드 Java target** = 17 확정? (§1)
2. **데이터 영속 분리** = 게임 NBT / 인증·통계 DB / config 토글 — 이 구조 맞나? `data-schema` DB와 연동? (§2)
3. **블록 내구 저장** = 좌표별 vs 영역 배율? (§2)
4. *(이하 모듈 설계하며 누적)*
