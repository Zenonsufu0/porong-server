...
import com.porong.gun.inv.WeightUtil;
...
/**
 * 프로토타입 검증용 커맨드.
 *   /porongun invtest        — 인벤 GUI 프로토타입 화면 열기
 *   /porongun setweight &lt;n&gt; — 손에 든 아이템 무게 태그 설정(TaCZ 총에 얹어 점유칸 확인)
 */
@Mod.EventBusSubscriber(modid = PorongunCore.MODID, bus = Mod.EventBusSubscriber.Bus.FORGE)
public final class PorongunCommands {

    private PorongunCommands() {}

    @SubscribeEvent
    public static void onRegister(RegisterCommandsEvent event) {
        event.getDispatcher().register(Commands.literal("porongun")
