...
> - 🔜 다음: **#1 제작법** — 서버 떠서 TaCZ 작업대·총·탄약·First Aid 의료템·Tactical 3D Armor 실레시피 확인 → `docs/design/economy.md`.
...
## 🔜 다음 세션 — 이어서 할 것
> 2026-06-08 세션에 **전체 컨셉 검토(A1~A3·B1·B3·B4·C) 완료.** 큰 설계 결정 거의 소진 → 남은 건 **밸런싱 수치**와 **결전 잔여 디테일**.
1. ✅ **결전 탈출 디테일 완료**(INBOX-032): 청사진 테크 트리(병렬 미션→부품 청사진[무선/연료/데이터, ChaosZPack POI 매핑]→고난도 제작→마지막 미션→티켓 청사진→조립). 추출=고정 지점+신호기 활성화(2-c). 청사진 영구 학습·와이프 리셋.
   - 🔜 남은 결전 수치: 미션 개수, 부품 희귀 재료, 헬기 좌석 수·추출 지점, 웨이브 점증, 시각대.
2. **미정 수치 채우기** (착수 임박 시 밸런싱): 화폐 드랍률·상점 가격, 식별 마우스오버 N초, 부위별 HP·페널티, 기지 레벨 단계 수치, Y25 초기화 타이밍, 무게 최대 감속, 굶주림 강도, 신규 보호막 12h 기준.
3. (선택) 칭호 외 추가 이월(다음 시즌 소량 부트스트랩) 여부 — 시즌2 운영·와이프 철학 확정 시.
