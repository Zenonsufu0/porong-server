...
>
> **🔜 다음 세션 (설계 이어가기):**
> 1. **관통 폭약** — 코어 동심방벽(1~3겹) 뚫는 공격. 폭약 종류·제작 비용(비파밍 게이트=레이드 빈도 통제)·방벽 겹 대비 위력. → base_raid.md/economy.md.
> 2. **구조 신호기(티켓) + 탈출 부품**(무선/연료/데이터) 레시피.
> 3. **나머지 커스텀**(배낭/보안함 업글·메뉴 아이템·커스텀 화폐) 레시피.
> 4. **#2 가격표·드랍률** — 군용 합금/전자 부품 드랍률, 상점가, 텅스텐/열화우라늄 정확 수량, 모드 레시피 치환 수량(밸런싱).
> 5. **방어구 HP 보너스 옵션**(총 치명적이라) 결정.
>
